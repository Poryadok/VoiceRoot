...
Канон для Figma, design tokens и Flutter. Продуктовое поведение и фазность остаются в `docs/features/` и [PLAN.md](../PLAN.md); этот документ описывает, **как** показывать уже описанные фичи в интерфейсе.
...
- **Discord second:** когда у Telegram нет подходящего паттерна, особенно для спейсов, дерева каналов, голосовых комнат, ролей, presence и community-flow, берём UX-логику Discord, но не копируем его визуальный шум.
...
- **Спокойная геометрия:** не "острый" UI, но и не pills везде. По умолчанию `radius.sm` (4px), для инпутов и compact controls допустим `radius.md` (6px), крупные радиусы только у аватаров, медиа-превью и редких карточек.
...
| Desktop shell, спейсы, голосовые комнаты, роли | **Discord** | Трёхколоночную структуру, дерево каналов, join/leave voice, presence, но без визуальной перегрузки. |
...
Phase 1: inline / side panel / bottom sheet; full-screen только где необходимо (Auth, mobile chat, critical flows).
...
- **Chat first:** открыть, прочитать, написать — основной happy path. В Phase 1 не добавлять UI-шум будущих фич.
...
- **DM from strangers:** до отдельной папки запросов (Фаза 3) не обещать отдельный inbox в UI; когда появится, паттерн ближе к Telegram requests: заметно, но не мешает основному списку.
...
Перед реализацией или Figma-экраном проверить:
