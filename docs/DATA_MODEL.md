# Модель данных — общие правила

Договорённости для описания таблиц и связей по сервисам: секция «Модель данных» в [docs/microservices/](microservices/) и миграции рядом с кодом сервиса ([OPERATIONS.md](OPERATIONS.md)). Инвентарь БД: [DATA_STORES.md](DATA_STORES.md).

**Скоуп первой итерации схем** (какие сервисы и сущности входят до полного каталога таблиц): [DATA_SCOPE_V1.md](DATA_SCOPE_V1.md).

---

## 1. Идентификаторы и границы account / profile

### Тип и формат

- Первичные ключи сущностей, которые уходят в API, JWT и в другие сервисы — **UUID** (в PostgreSQL тип `UUID`). По умолчанию генерация **UUIDv4** через `gen_random_uuid()` в миграциях/БД или эквивалент в приложении.
- **Исключение:** идентификаторы строк в `messaging_db.messages` — **только UUIDv7**, выставляемые **приложением Messaging (Go)** при вставке; ULID и прочие форматы не применяем. Детали — [microservices/messaging-service.md](microservices/messaging-service.md#модель-данных).
- Внутренние суррогатные ключи допускаются только там, где ID наружу не отдаётся (редкие технические таблицы); по умолчанию не вводим второй «внутренний» ключ без причины.

### Аккаунт и профиль

| Понятие                    | Где живёт                                          | Назначение                                                                                         |
|----------------------------|----------------------------------------------------|----------------------------------------------------------------------------------------------------|
| **Аккаунт** (`account_id`) | **Auth Service**, БД `auth_db`, таблица `accounts` | Логин, пароль, 2FA, refresh, email/телефон, soft delete, тип regular/guest                         |
| **Профиль** (`profile_id`) | **User Service**, БД `user_db`                     | Ник, аватар, приватность, presence, мульти-профиль — [multi-profile.md](features/multi-profile.md) |

### JWT и имена в claims

В access token (и в gRPC metadata после Gateway) используются claims из [api-gateway.md](microservices/api-gateway.md):

| Claim в JWT                  | Смысл                                  | Соответствие в БД                                                |
|------------------------------|----------------------------------------|------------------------------------------------------------------|
| `user_id`                    | Идентификатор аккаунта (сквозная auth) | **`accounts.id`** = `account_id`; имя claim исторически короткое |
| `profile_id`                 | Активный профиль для этого запроса     | **`profiles.id`** в `user_db`; момент появления первичного профиля и выдача JWT без «окна» без `profile_id` — [microservices/primary-profile-bootstrap.md](microservices/primary-profile-bootstrap.md) |
| `session_epoch`               | Версия сессий аккаунта для отзыва всех сессий (T056-P1) | **`accounts.session_epoch`** в `auth_db`; положительный integer |
| `roles`, `subscription_tier` | Кэш для Edge                           | Source of truth — Role / Subscription; без лишних дублей в БД    |

#### T056-P1: отзыв всех сессий

Источник истины — `auth_db.accounts.session_epoch`:

- `session_epoch BIGINT NOT NULL DEFAULT 1`; значение всегда положительное.
- Auth увеличивает его атомарно при операции, которая отзывает все сессии
  аккаунта. Значение монотонно растёт и никогда не уменьшается, в том числе при
  повторе операции или восстановлении из более старого состояния БД.
- Каждый новый access JWT получает положительный integer claim `session_epoch`.
  `jti` остаётся идентификатором конкретного access JWT и продолжает служить
  для более узкого отзыва одной сессии.
- Для edge-проверки Auth зеркалирует минимум epoch аккаунта в Redis без TTL;
  запись обновляется только вверх. Redis — быстрый enforcement-store, но не
  замена durable источнику в Auth DB. Ошибка чтения, повреждённое значение или
  отсутствие floor в strict-режиме отклоняются (missing floor **не** означает
  `1`).

В Compose strict-потребители Gateway и Realtime включены после Auth migration,
startup seed и issuance preparation: отсутствующий/неположительный claim и
отсутствующий/невалидный floor отклоняются fail-closed. Compatibility допустим
только в явно настроенном окружении вне доказанного strict deployment. Compose
strict proof не завершает rollout во всех окружениях; account-targeted закрытие
через Redis Pub/Sub не реализовано.

В **новых** миграциях Auth предпочтительно колонка **`account_id`** там, где логически ссылка на `accounts.id` (в т.ч. в `refresh_tokens`). В [ARCHITECTURE_REQUIREMENTS.md](ARCHITECTURE_REQUIREMENTS.md) в примере таблицы указано имя `user_id` — при проектировании схемы трактуем его как **тот же идентификатор, что и `account_id`**, и выравниваем имя колонки к `account_id` ради однозначности.

**Protobuf:** в телах gRPC-сообщений для ссылки на аккаунт используем **`account_id`**, кроме полей, которые намеренно зеркалят JWT (`user_id` в `TokenClaims` и т.п.) — см. раздел Protobuf в [REPOSITORIES.md](REPOSITORIES.md).

### Что в какой БД не кладём

- В **`auth_db`** нет профилей, ников, друзей — только всё, что относится к учётной записи и сессиям.
- В **`user_db`** нет хешей паролей и refresh tokens; хранится **`account_id`** как внешняя ссылка (UUID без FK на другой кластер БД).

---

## 2. Ссылки между сервисами (без FK между БД)

### Правило

- У каждого микросервиса **своя** PostgreSQL БД ([DATA_STORES.md](DATA_STORES.md)). **Между разными БД внешние ключи (FK) не задаём** — СУБД не видит чужие таблицы.
- Связь «сущность A ссылается на сущность из сервиса B» выражается **колонкой UUID** (например `account_id`, `profile_id`, `chat_id`) плюс документированное правило: *валидность и жизненный цикл обеспечивает владелец B через API / события*.

### Нотация в документации

Во всех секциях «Модель данных» используем единый формат для межсервисной ссылки:
- `UUID, logical ref → <service_db>.<table>.<column>; без меж-БД REFERENCES`

Если ссылка внутри той же БД/схемы, допускается реальный `REFERENCES` в миграции.

### На практике

| Колонка ссылается на     | Владелец данных     | Проверка целостности                                                     |
|--------------------------|---------------------|--------------------------------------------------------------------------|
| `account_id`             | Auth                | User/Social + Auth или события; удаление аккаунта — см. продуктовые доки |
| `profile_id`             | User                | Через API и доменные правила                                             |
| `chat_id`, `space_id`, … | Chat / Space и т.д. | UUID в чужой БД; FK только внутри своей БД                               |

Дублирование справочных полей (например кэш отображаемого имени) допускается осознанно для чтения; источник истины — сервис-владелец.

### Уровни идентификаторов в deep links и API

- Для профиля в публичных ссылках используем `username` (или `profile_id`, если сценарий строго внутренний), а не `account_id`.
- Для DM ссылка уровня пользователя должна быть однозначно привязана к **профилю** (`profile_id`) или к `username`; смешивать `userId` (как alias `account_id`) с DM-адресацией профиля нельзя.
- Для текстовых чатов всегда используем `chat_id = chat_db.chats.id`; для голосовых комнат — `voice_room_id = space_db.voice_rooms.id`.

### Группа и канал (`chat_type = group` \| `channel`)

**Группа** и **канал** — одна и та же техническая сущность: строка в **`chat_db.chats`** с `type = group` или `type = channel`. Разница — **дефолтные настройки** и пресеты ролей; смена `type` с `channel` на `group` (и обратно) должна оставлять чат рабочим — меняются только дефолты и проверки прав.

- **Дефолт канала:** в основную ленту **нельзя** отправлять сообщения **от имени пользователя** (только «от имени чата» / треды — по настройкам). **Дефолт группы:** в основную ленту **можно** от имени пользователя; «от имени чата» — опционально (настройки + роли). Реальный автор всегда в `sender_profile_id`; режим отображения — поля `posted_as_chat` / `display_chat_id` в Messaging (см. [microservices/messaging-service.md](microservices/messaging-service.md#модель-данных)).
- Чат **без спейса** (`space_id` NULL): участники в **`chat_db.chat_members`**. Чат **в спейсе**: состав **наследуется** от **`space_db.space_members`**; спейс через **Role** задаёт роли и **оверрайды** по `chat_id` / `voice_room_id`, чтобы ограничивать доступ к конкретным чатам и голосовым комнатам.
- Чат в спейсе появляется в sidebar через узел **`space_db.space_tree_nodes`** с `kind = text_chat` и ссылкой на **`chat_db.chats.id`** (`group` \| `channel`): там же задаются **категория**, **`sort_order`** и флаг **`is_system`** (системный чат объявлений). Сообщения по-прежнему в Messaging с тем же `chat_id`.

- В **Messaging** / **File** / **read_receipts** поле **`chat_id` всегда = `chat_db.chats.id`** для `dm` \| `group` \| `channel`.
- **`chat_type`** в `messaging_db.messages` дублирует `chats.type` для индексов и политик; семантика ленты/тредов задаётся **настройками чата**, а не жёстко типом.

**Права:** standalone чат — **Chat** + настройки. Чат с `space_id` — **Space** (дерево `space_tree_nodes`), **Role** (роли участника спейса + `chat_overrides` / `voice_room_overrides`). Messaging не вводит FK на чужие БД и опирается на решение Gateway/оркестрации.

### Дерево спейса (`space_tree_nodes`)

**Один слой сортировки** для боковой панели: текстовые чаты и голосовые комнаты — строки в **`space_db.space_tree_nodes`** с полем **`kind`** (`text_chat` \| `voice_room`), общими **`category_id`** и **`sort_order`**. Сущность голосовой комнаты (`name`, `space_id`) хранится в **`voice_rooms`**; позиция в дереве **только** в `space_tree_nodes` (у `voice_rooms` нет собственных `category_id` / `sort_order`). Подробнее — [microservices/space-service.md](microservices/space-service.md#модель-данных).

### Голосовые комнаты в спейсе

Идентификаторы **голосовых комнат** живут в **`space_db.voice_rooms`** (отдельно от текстового потока Messaging). К **Messaging** они по умолчанию не привязаны. Оверрайды прав Role для голоса используют отдельный `voice_room_id = voice_rooms.id` в том же спейсе (см. [microservices/role-service.md](microservices/role-service.md#модель-данных)). Узел sidebar с `kind = voice_room` ссылается на тот же `voice_rooms.id`.

---

## 3. Общие поля, время и soft delete

### Временные метки

- **`created_at`** — `TIMESTAMPTZ`, UTC, не обнуляется.
- **`updated_at`** — `TIMESTAMPTZ`, UTC, обновляется при изменении строки (триггер или слой приложения). Для стратегии **last write wins** по доменным правилам — см. [ARCHITECTURE_REQUIREMENTS.md](ARCHITECTURE_REQUIREMENTS.md) (конкурентные операции).

Имя колонок в БД — **snake_case**.

### Soft delete

- Где в архитектуре задано soft delete (например **удаление аккаунта** — [ARCHITECTURE_REQUIREMENTS.md](ARCHITECTURE_REQUIREMENTS.md)), используем **`deleted_at TIMESTAMPTZ NULL`**: `NULL` = активная запись, непустое значение — удалена в логическом смысле.
- Для сущностей без требования «хранить после удаления» допускается жёсткое удаление или отдельная политика — фиксировать в описании конкретной таблицы.
- Если в таблице одновременно есть `status` и `deleted_at`, источником истины для факта удаления считаем `deleted_at`; `status='deleted'` — производный доменный маркер и должен быть согласован с `deleted_at IS NOT NULL`.

### Кросс-сервисная ретенция и удаление

- Политики retention (файлы/сторис/сообщения) задаются владельцем домена и документируются в соответствующем сервисе; в чужих БД храним только UUID-ссылки и технические метки, без попытки дублировать source of truth.
- Удаление аккаунта/профиля должно иметь документированную реакцию в сервисах-консумерах (анонимизация, запрет новых действий, скрытие сущностей), но реализуется через API/события, а не через FK между БД.

### Прочие повторяющиеся поля

- Вводить **единый стиль** для статусов перечислением (`status` + `CHECK` или отдельный тип), а не свободные строки — в спецификации каждой таблицы перечислить допустимые значения.
- **`id`** как UUID первичный ключ — по умолчанию для всех публичных сущностей (см. п. 1).

---

## Следующие шаги

- Детальные таблицы по сервисам: секции «Модель данных» в [microservices/](microservices/); объём первой волны миграций — [DATA_SCOPE_V1.md](DATA_SCOPE_V1.md); инвентарь БД — [DATA_STORES.md](DATA_STORES.md).
- После стабилизации черновика — короткая отсылка из этого файла в [ARCHITECTURE_REQUIREMENTS.md](ARCHITECTURE_REQUIREMENTS.md) (по желанию), чтобы не дублировать длинные абзацы.

## P3 Space deletion ownership and File references (accepted target)

Space owns lifecycle state, generation, deadline, immutable root manifest and
participant receipts; each participant owns its fence, cleanup rows and receipt
in its own database. No coordinator reads another service database. The fixed
participant set is Role, Chat, Messaging, File, Voice, Matchmaking, Search,
Subscription, Bot and Notification. Lower generations are stale no-ops; same
generation with changed state/bytes conflicts; `PURGE_DECIDED`/`PURGED` can
never return to `LIVE`.

After purge, ordinary Space rows and audit rows are removed. A separate no-FK
`space_deletion_tombstones` row keeps only Space ID, purpose-specific HMAC
owner/actor account values, key version, `OWNER_REQUESTED`, lifecycle times and
`retain_until = purged_at + 365 days`; no P3 legal hold exists. Role and every
participant keep permanent identity-reuse fences containing no account/profile
identity. Moderation, Analytics and Subscription follow their explicit service
classifications rather than becoming coordinator completion participants.

File owns cross-service reference liveness. A live reference is identified by
`(file_id, owner_type, owner_id, subresource_id?, scope_space_id?)`; the
authenticated owner supplies `scope_space_id`, File does not infer it. Every
Space-derived reference requires that scope. A URL/metadata/bulk read requires
one exact live reference or a subject-bound capability and checks the File-owned
Space fence before any uploader/owner shortcut. Owner rows become externally
visible only after durable reference acquisition, and release names the exact
same tuple. Blob GC begins only when File proves zero live references globally.
