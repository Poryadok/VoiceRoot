...
**Последний проход:** 2026-06-19 — третий проход (post–Batch 4–7 contracts): выборочная сверка [MICROSERVICES.md](MICROSERVICES.md) Event Bus ↔ [CONTRACT_MATRIX.md](CONTRACT_MATRIX.md) (`story.events`, `bot.events`, `social.events`, `message.events` — publishers/subscribers совпадают); HTTP-префиксы Gateway (`/bots`, `/stories`, `/matchmaking`, `/ws`) — [api-gateway.md](microservices/api-gateway.md) ↔ матрица; Realtime без PostgreSQL — согласовано с [DATA_STORES.md](DATA_STORES.md); политика codegen protobuf — таблица в [REPOSITORIES.md](REPOSITORIES.md) (`lib/gen`, `*/pb/`, `buf-dart-check`, `gen/go` gitignored). **Открыто:** `e2e_key_backups` в инвентаре `auth_db` — [DATA_STORES.md](DATA_STORES.md) (см. [TODO.md](TODO.md) Batch 1); `story.lfp_created` → Matchmaking subscriber — deferred в [story-service.md](microservices/story-service.md).

**Ранее:** 2026-04-16 — второй проход (integrity audit): сверка [MICROSERVICES.md](MICROSERVICES.md) ↔ [CONTRACT_MATRIX.md](CONTRACT_MATRIX.md) ↔ [microservices/api-gateway.md](microservices/api-gateway.md) (HTTP-префиксы, `/ws`, Federation вне Gateway); таблица JetStream в матрице и Event Bus — совпадают; в блок «Владение данными» добавлена строка **Realtime Service** (Redis / без PostgreSQL) для согласованности с [DATA_STORES.md](DATA_STORES.md); полный прогон `markdown-link-check` по `docs/` с [.markdown-link-check.json](../.markdown-link-check.json) (игнор `https://` и `#`-якорей); CI: [.github/workflows/docs-link-check.yml](../.github/workflows/docs-link-check.yml); [TESTING.md](TESTING.md) обновлён. Ранее в тот же день: `social.events` / `role.events`, карточки NATS/идемпотентность, stories/search/reports, Bot rate limits и версии клиента в [ARCHITECTURE_REQUIREMENTS.md](ARCHITECTURE_REQUIREMENTS.md), [TODO.md](TODO.md).
