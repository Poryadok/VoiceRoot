...
- Публичные **FQDN** на этом стенде — [`deploy/staging/domains.defaults`](../deploy/staging/domains.defaults) (`voice.comrade.click`, `developers.comrade.click`, `app.comrade.click`). В GitHub: переменные **`VOICE_GATEWAY_INGRESS_HOST`** / **`VOICE_DEVELOPER_PORTAL_INGRESS_HOST`** / **`VOICE_WEB_INGRESS_HOST`**. DNS: в Cloudflare для зоны **comrade.click** — записи **A** для поддоменов **`voice`**, **`developers`** и **`app`** на внешний IP этой ноды (см. таблицу «Хост и SSH» выше). TLS: **Cloudflare Flexible SSL** — HTTPS до пользователя, HTTP до origin; детали — [DEPLOYMENT.md](DEPLOYMENT.md).
