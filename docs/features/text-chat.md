...
### Индекс тредов и постраничная выдача

`ListThreads` — отдельный index тредов для одного уже доступного `chat_id`; он
не заменяет историю `GetThreadMessages`. Первая страница фиксирует high-water
по `messages.created_at` в БД. Поэтому reply, созданный после первого запроса,
попадает только в новый snapshot, а не меняет состав последующих страниц
текущего cursor. Это намеренно узкая гарантия стабильности набора; она не
замораживает состояние видимости: `FOR_EVERYONE`, `FOR_ME` для данного viewer и
`ghost_only` применяются при чтении каждой страницы немедленно. Ghost root или
reply виден только его sender; другой участник не получает ни thread summary,
ни preview, ни ID из cursor.
...
### Права
- Создатель standalone-группы = владелец; назначает и снимает администраторов, передаёт ownership участнику.
- Администратор standalone-группы может назначать администраторов из участников, но не может снимать админство у другого администратора; owner может.
- Администраторы standalone-группы добавляют и удаляют обычных участников, редактируют настройки группы и удаляют сообщения; admin не удаляет owner или другого admin из группы.
- При передаче ownership новый владелец получает роль `owner`, прежний владелец становится `member`.
- Роли group/channel внутри Space задаёт Space/Role Service: admin по умолчанию имеет все права, кроме удаления Space, передачи ownership и снятия admin у другого admin.
