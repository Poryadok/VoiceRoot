...
> **Proto gap:** `send_silent`, `schedule` oneof, scheduled RPCs/table and schedule result union — **not yet in proto/code**. PR #310 owns `message.sent.send_silent = 8`; follow-up scheduled metadata uses fields 9–10 only after that PR. Normative contract — [messaging-service.md](../microservices/messaging-service.md) § Send options, § Scheduled messages ([todo/backend.md](../todo/backend.md)).
