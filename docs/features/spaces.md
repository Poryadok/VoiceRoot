...
- **Текстовые каналы** — строки в **Chat** (`chats`, `type = channel`); в дереве спейса — плейсмент (`space_text_channel_placements`)
- **Голосовые комнаты** — `voice_rooms` в **Space**
- **Группы** — `chats`, `type = group`, с `space_id`
...
- Создание / удаление / переименование **текстового канала** (Chat), **голосовой комнаты**, **группы** в спейсе
