...
Для входа без связи применяется отдельная игровая учётная запись, описанная
...
2. Игрок подключает выбранный профиль Voice либо использует будущую ограниченную
   игровую личность, если эта capability включена для приложения.
...
- Ограниченная игровая личность — будущая capability с отдельной политикой.
...
### Игровая учётная запись и переход в постоянную

**Принято владельцем:** это отдельный тип identity, не текущий гостевой аккаунт
Voice и не переименование `guest`. Основание доверия — проверенное владение
игровым аккаунтом через доверенный backend/provider. Оно позволяет участвовать
...
game subject в том же app/env возвращает ту же игровую identity. Срок хранения
...
- **В существующий постоянный аккаунт:** пользователь доказывает владение игровой
  identity и аккаунтом Voice, выбирает профиль, видит переносимые связи и
...
[Game Integration API](../architecture/game-integration-api.md#конвертация-игровой-identity).
