...
5. Replace `RunArchivePurgeOnce` pre-delete File calls with staging plus a startup/short-interval dispatcher. A dispatcher claims one ready or expired row per tick using a token/fenced lease, calls File only after the Story is absent, deletes only its own completed lease, and backs off failures or ambiguous outcomes with capped exponential DB-time delays. Every File call has a bounded deadline, so an unresponsive client releases its lease for a later tick without pre-leasing later rows. File deletion is idempotent by immutable File ID.
...
| Multi-operation deadline cadence | Two ready operations use a cooperative `DeadlineExceeded` deleter on the first tick. | Only one row is leased and failed; the other remains unattempted and ready, then is completed by the next tick without a stale-token File call. |
