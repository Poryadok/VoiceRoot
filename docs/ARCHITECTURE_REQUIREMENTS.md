...
- **Shipped scope (durable delivery / list ticks):** ephemeral WS path and the durable DM path are **shipped**: `last_delivered_message_id`, JetStream `message.delivery_ack` consumer, and `GetChatListMetadata.last_message_delivery_state`. Group/channel list delivery ticks remain `none`.
...
**Shipped scope:** `MarkRead` / durable read cursor validation — **DM today**; group/channel read parity — backlog. List delivery ticks (`last_message_delivery_state`) are shipped for DM; group/channel ticks remain `none`.
