...
**`message.sent` notes:** `send_silent` drives Notification push policy. `was_scheduled` / `scheduled_at` — audit и client strip cleanup. `content_type` and `send_silent` are in the JetStream proto and Messaging producer; scheduled metadata remains unimplemented.
