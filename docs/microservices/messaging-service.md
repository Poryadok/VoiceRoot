...
`SendMessage` принимает опциональный **`client_message_id`** (UUID), уникальный в разрезе **`(chat_id, sender_profile_id)`**. Повтор запроса с тем же ключом после таймаута не создаёт второе сообщение: ответ — существующая строка `messages` (код **идемпотентного успеха** фиксируется в gRPC/OpenAPI при генерации контракта). Без ключа возможны дубликаты при ретрае клиента.
