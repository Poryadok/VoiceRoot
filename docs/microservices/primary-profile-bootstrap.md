...
Первичная строка в `user_db.profiles` с `is_primary = true` появляется **в том же запросе**, в котором Auth впервые выдаёт клиенту пару access + refresh после успешной аутентификации (или после успешного **refresh**). Точки входа продукта Фазы 1:
...
Для **всех** успешных ответов Auth Фазы 1, которые возвращают **`access_token`** (register / login / refresh):
...
**Скоуп:** речь о выдаче токенов Auth Фазы 1 с включённым провижинингом в `user_db` (или эквивалентном вызове **EnsurePrimaryProfile** у User Service). Произвольные старые токены или аварийные обходы не задают контракт продукта.
