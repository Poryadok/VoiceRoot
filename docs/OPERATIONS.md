...
- **Инструменты:** Flyway (Auth / Java), golang-migrate (Go-сервисы) — [data/README.md](data/README.md#db-migrations).
...
- **Удаление спейса и проекции**: публикуется доменное событие (например `space.deleted`); Role, Search и другие сервисы удаляют связанные строки **идемпотентно** (см. [data/target/role_db.md](data/target/role_db.md)). Порядок относительно Messaging и TTL сообщений канала — явно в дизайне фичи (сага / transactional outbox).
