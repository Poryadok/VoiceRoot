...
| sdk-account | Отдельный от текущего guest тип аккаунта Voice для SDK-интеграции с проверенным game subject и ограничениями app/env/scopes; повторно используется между входами, конвертируется в новый или существующий постоянный аккаунт |
