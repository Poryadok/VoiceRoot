...
|--------------------------|----------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Основной поток**       | Сообщения в ленту пишут **участники** (от своего имени).                                                                         | В основной ленте публикует **сущность канала** (официальные посты); участники обсуждают в **тредах** у постов, а не в общей ленте. Технически реальный автор всё равно хранится как `sender_profile_id`, а "от имени канала" — отдельный признак отображения. |
...
| **Где живёт сущность**   | `chat_db.chats` с `type = group`; при спейсе — **`space_id`** → `spaces.id`; возможна группа **вне спейса** (`space_id` пустой). | `chat_db.chats` с `type = channel` (имя/аватар сущности канала, `space_id` пустой **или** спейс); в дереве спейса — строка в **`space_db.space_text_channel_placements`** → тот же `chats.id`.                                                                |
...
**Спейс** — контейнер **только для групповых чатов**: **текстовые каналы** (строки `chats` + плейсмент в дереве), текстовые **группы** (с `space_id`), **голосовые комнаты** — в любой комбинации; публичный или приватный по каталогу — см. [spaces.md](features/spaces.md). Текстовый канал **может существовать и без спейса** — тогда нет строки в `space_text_channel_placements`.
...
| Текстовый канал (вне или в спейсе)   | `channel`                                           | `chat_db.chats` (`type = channel`); в спейсе + `space_text_channel_placements` |
| Голосовая комната в спейсе           | — (не Messaging)                                    | `space_db.voice_rooms` + Voice                                                 |
