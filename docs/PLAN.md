...
Репозиторий на этапе **документации и фундамента Фазы 0**: спецификации, контракты, инфраструктурный каркас, **API Gateway** на Go (REST/WS edge proxy, JWT/JWKS, Redis blacklist/rate limit, CORS, `/metrics`, version policy, тесты, Dockerfile) и **инициализированные backend-проекты микросервисов**. **CI на GitHub**: на push в `master` образы **всех** Go-сервисов из матрицы и **Auth** пушатся в GHCR; **деплой gateway на staging** проверен end-to-end ([DEPLOYMENT.md](DEPLOYMENT.md)); публичный FQDN стенда — `voice.tastytest.online`. В **Auth** реализованы register/login/refresh/logout/validate (REST+gRPC), JWT/JWKS; продовый wiring: **PostgreSQL** (JDBC + Flyway `V1__auth_schema.sql`), **Redis** для blacklist по `jti`, **стабильный JWKS** из PKCS#8 PEM (`auth.jwt.private-key-pem` / `auth.jwt.private-key-location`), образ **EXPOSE 8080 9090**; профиль `test` оставляет in-memory репозитории и ephemeral JWT для быстрых тестов; интеграция JDBC+Redis — `AuthJdbcRedisIntegrationTest` (Testcontainers, на CI с Docker; в `docker run … mvn` без сокета Docker тест пропускается). `mvn -B test` и сборка Docker-образа проходят. Доменные реализации **Realtime/Messaging** по-прежнему не готовы: Go-сервисы на уровне scaffold с health-check, тестами и Dockerfile. **Flutter-клиент** ([`src/frontend/`](../src/frontend/)): Phase 0 shell — трёхколоночный layout, Riverpod (DI/state), тонкий HTTP к публичным эндпоинтам Gateway (`/health`, `/api/v1/version`), `flutter test`/`flutter analyze`, отдельный job в CI; продуктовый UI (логин, чаты, Realtime) — следующие фазы.
...
| CI / staging | Workflows: [ci.yml](../.github/workflows/ci.yml), [staging-deploy.yml](../.github/workflows/staging-deploy.yml), [docs-link-check.yml](../.github/workflows/docs-link-check.yml) — см. [TESTING.md](TESTING.md), [DEPLOYMENT.md](DEPLOYMENT.md). На push в `master` в GHCR пушатся образы матрицы Go-сервисов и Auth; выкат на staging end-to-end проверен для **gateway**. |
...
| - [ ]  | 2    | Звонки 1:1                                         | Web                   |
| - [ ]  | 3    | Вложения, typing, статусы, DM «запросы», edit/delete UX | Web                   |
...
- [ ] **DM — папка «запросы»** для сообщений от незнакомцев (не входило в Фазу 1) — [friends.md](features/friends.md); Chat/Messaging + клиент: отдельный список / инбокс, пометка «незнакомец»
