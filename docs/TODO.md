...
### Batch 15 — Product roadmap (20 improvements)

Источник: план *20 product improvements* (2026-07). Спеки: [docs/features/](features/). **П.7 мультипрофиль — закрыт** (presets, accent, voice-preserving switch). Пересечения с другими batch — ссылка, не дублировать детали Batch 14.

**Промпт-якорь:** `Product roadmap Batch 15 from docs/TODO.md` + `TDD: <фича> per docs/features/<spec>.md`.

#### High — матчмейкинг и discovery
...
#### Common — trust, accounts, engagement
...
Selective build/deploy в `master` ([`ci.yml`](../.github/workflows/ci.yml), [`staging-deploy.yml`](../.github/workflows/staging-deploy.yml), [`DEPLOYMENT.md`](DEPLOYMENT.md)). Ниже — открытые ops-задачи, баги pipeline и осознанный техдолг.
...
### Batch 14 — Full product audit (2026-07-14)

Параллельный аудит 26 модулей (детальные gap-ы по сервисам; дубликаты Batch 1–15 и roadmap не повторяются).
...
- [ ] **[Subscription] Premium cosmetic gaps outside Subscription module — e.g. custom status not persisted; anonymous view tracked separately in `docs/TODO.md`** — `src/backend/user/internal/grpcsvc/user.go`; `docs/TODO.md` (Anonymous view)
...
- [ ] **[Federation] `docs/TODO.md` L257 partially stale** — claims no staging Deployment; `deploy/staging/services.yaml` now has `voice-federation`. Still correct that `deploy/staging/configmap-app.yaml` / `deploy/prod/configmap-app.yaml` omit federation from `GATEWAY_GRPC_UPSTREAMS_JSON`.
...
**Промпт-якорь:** Full product audit Batch 14 from docs/TODO.md.
...
### Batch 12 — Windows / phase→features follow-up

Миграция phase→features завершена ([`apply-phase-text-replacements.ps1`](../scripts/dev/apply-phase-text-replacements.ps1)).
...
| **Common** | Batch 15 | Product roadmap (19 пунктов плана) |
| **Common** | Batch 11 | CI/CD, staging/prod deploy |
| **Common** | Batch 14 | Full product audit 2026-07-14 (по модулям) |
| **Low** | Batch 7 | Stories post-MVP |
| **Low** | Batch 10b | Magic numbers → env/ConfigMap |
| **Low** | Batch 12 | Windows sign-off |
