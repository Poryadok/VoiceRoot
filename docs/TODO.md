...
Здесь — **вне дорожной карты** [PLAN.md](PLAN.md). Продуктовые и инфраструктурные чеклисты по фазам, остаток Фазы 0 (Auth: smoke контейнера в CI, порядок миграций `auth_db`), NATS/JetStream в Compose, UUIDv7 для сообщений — в плане. Крупные темы при желании дублируйте в issue-трекере.
