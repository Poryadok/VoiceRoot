...
| Голос/видео, File Service, R2 | Фазы 2–3+ |
...
Для **User** в v1 — **профиль** и базовые поля; **мульти-профиль** (несколько `profiles` на аккаунт как продуктовая фича) и расширенные Premium-поля — по [PLAN.md](PLAN.md) ближе к Фазам 12–13; технически одна строка `profiles` с `is_primary = true` на аккаунт достаточна для Фазы 1.
...
- `messages` — идентификатор, `chat_id`, отправитель `sender_profile_id`, текст, метки времени, soft delete при необходимости; без тредов/реакций/пинов в первой миграции (или с nullable полями под будущее — решение в `docs/data/messaging-service.md`).
...
| [friends.md](features/friends.md) | Social, User | Заявки и друзья → `friendships`; блоки → `blocks` |
...
| [text-chat.md](features/text-chat.md) | Chat, Messaging, Realtime | Только подмножество без реакций/тредов/пинов в Фазе 1 |
...
1. Для каждого сервиса из п. 3 — файл **`docs/data/<service>.md`**: таблицы, PK/FK внутри БД, UUID-ссылки наружу, индексы, отложенные поля.
2. Сверить с [OPERATIONS.md](OPERATIONS.md) (expand/contract, владелец миграций).
3. Обновить [PLAN.md](PLAN.md) Фазу 0: миграции создаются для перечисленных выше БД, а не под устаревшие имена «users, conversations» из одной монолитной схемы.
