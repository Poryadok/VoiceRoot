...
**Аватар и R2 без `file_db`:** загрузка **только** статичного аватара профиля — presigned upload в R2 и запись ссылки в `profiles`; без таблиц File Service и без общего пайплайна вложений (детали и границы — [PLAN.md](PLAN.md) Фаза 1, секция «R2 / аватар»). Вложения в чате, метаданные файлов в БД, антивирус и т.д. — **Фаза 3+**, см. [file-service.md](microservices/file-service.md).
...
| [user-profile.md](features/user-profile.md)           | User                       | Отображаемое имя; **аватар** — URL после загрузки в R2 (**Фаза 1**, минимальный presigned-контур без `file_db`); **вложения в чате и полный File Service** — Фаза 3 |
