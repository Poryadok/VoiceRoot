...
- [ ] **[Space/Auth] Implement the approved resource-bound step-up proof for ownership transfer.** The canonical contract now fixes password plus conditional TOTP/backup code, opaque hash-only proof, exact account/profile/space/target/operation/session/factor bindings, five-minute TTL, revocation triggers, atomic Space-only one-use consume with same-operation durable receipt, and fail-closed public errors. Current protos and handlers contain none of it. Add both service APIs/storage/S2S enforcement and negative/replay/compensation tests as one slice; do not promote the current two-ID transfer handler to a public path. — [spaces.md](../features/spaces.md#контракт-подтверждения-передачи-владения); [auth-service.md](../microservices/auth-service.md#ownership-transfer-step-up-proof-target-contract-not-implemented); [space-service.md](../microservices/space-service.md#ownership-transfer-contract-target-not-implemented); `protos/voice/{auth,space}/v1/*.proto`
