...
- [ ] **[Story/File] Protected Story-media admission remains unimplemented** — the additive `ValidateStoryMedia` contract requires File-owned atomic locked-row validation and a strict `service:story` Phase-0 TLS/JWKS/replay boundary; Story must use the attestation for closed photo/video input and must never forward Gateway identity or call `GetFileMetadata`. RED matrices cover verified-principal/binding and File predicate cases plus closed Story/LFP no-write/no-event cases; they intentionally fail until this runtime exists. Media-bearing LFP fails closed until a product enum/duration policy exists; exact Story-ID claim/reference lifecycle is separate follow-up work. Paths: `protos/voice/file/v1/file.proto`, `src/backend/file/internal/grpcsvc/`, `src/backend/story/internal/grpcsvc/`.
