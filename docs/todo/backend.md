...
- [x] **[Analytics] `role_events` stream consumed** — **done:** PR #238 registers `role_events` / `role.>` with `handleRoleMsg`; runner tests cover startup and missing-stream retry (`src/backend/analytics/internal/consumer/runner.go`, `runner_test.go`).
