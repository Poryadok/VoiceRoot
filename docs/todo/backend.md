...
- [ ] **[Messaging] `GetChatListMetadata` preview DTO** — add `content_type`, `delivery_state`, `is_outgoing`; doc in messaging-service.md; **partial:** `last_message_is_outgoing` + `last_message_delivery_state` shipped (Batch 12); `content_type` still open.
...
- [ ] **[Messaging] Durable delivery consumer** — **done (Batch 12):** Realtime JetStream `message.delivery_ack` publish + Messaging consumer → `last_delivered_message_id`; list ✓✓ via `GetChatListMetadata.last_message_delivery_state`.
