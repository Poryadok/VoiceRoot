...
- [ ] **[A2 contract / Space↔Voice] Trusted S2S identity for `ResolveVoiceRoomAccess` is unspecified** — the proposed resolver must not accept caller-controlled forwarded profile metadata, but the authenticated calling-service identity, credential transport, and Space-side authorization owner are not defined. **Unblock:** W01/Space contract owner specifies the authenticated S2S principal and resolver authorization before Voice replaces the paged `ListMembers` adapter. Source: `tmp/fleet/slave-driver/a2-20260907/A2-P2-acl-design.md` § Required contract changes 1–2.
- [ ] **[A2 contract / Gateway↔Voice] Canonical resolver failures lack a stable public error mapping** — the ACL design distinguishes undiscoverable room from ownership/membership denial and dependency outage, but does not freeze the direct-gRPC and Gateway REST status/body mapping or which outcomes may disclose room existence. **Unblock:** W01 records one transport-consistent mapping for missing room, assertion mismatch, membership/permission deny, malformed resolver response, and unavailable Space/Role, with side-effect-free denial tests. Sources: `tmp/fleet/slave-driver/a2-20260907/A2-P2-acl-design.md` § Decision and scope, ACL-01–04; `tmp/fleet/a2-execplan-draft.md` A2-Gateway.
- [ ] **[A2 follow-on / Voice↔LiveKit] Canonical media grant and revocation contract is open** — after the resolver seam is frozen, A2 still needs signed claims/session binding, grant expiry, token-epoch/reissue behavior, and the revoke-to-disconnect guarantee after membership, `VOICE_JOIN`, or `VOICE_SPEAK` loss. **Unblock:** shared contract owner and Voice/LiveKit owner freeze those fields and bounded revocation behavior, then cover token issuance, reconnect, active publish, and forced removal. Sources: `tmp/fleet/slave-driver/a2-20260907/A2-P2-acl-design.md` § Required contract changes 6; `tmp/fleet/slave-driver/a2-20260907/A2-P10-space-voice-lifecycle-worker.md` §§ Current-state gap, Entry/token/reconnect invariant.
- [ ] **[A2 follow-on / Realtime↔Role] Space voice roster visibility authorization is unresolved** — after the resolver seam is frozen, the feature canon still needs the authoritative permission, redaction behavior, and requester/snapshot/event policy for a roster visible without joining. **Unblock:** Role/Realtime contract owner names the permission and deny/redact semantics, then makes `GetVoiceStates` and room-watcher fan-out enforce the same rule. Sources: [voice-chat.md](../features/voice-chat.md) § UX и статусы; `tmp/fleet/slave-driver/a2-20260907/A2-P2-acl-design.md` ACL matrix (roster); `tmp/fleet/a2-roster-session-events-proposal.md` § Realtime and client consumer contract.
