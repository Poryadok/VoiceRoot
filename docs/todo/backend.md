...
- [ ] **[A2 contract / Gateway↔Space] Public lifecycle, invite, tree, and audit route contracts are absent** — product and Space-service documents establish the underlying actions, audit access rule, ordering, and cursor only; they do not define the public method/path, JWT requirement, request/response JSON, `error_code`/disclosure, idempotency, or pagination projection for leave/transfer/delete/restore, invite create/list/revoke/redeem, tree/category/voice-room/node/pin mutations, or audit read/filter. Gateway currently has implementation-specific transcoders and tests for selected invite and tree routes; those are characterization evidence, not a canonical public API. `GetAuditLog`, lifecycle operations, tree pin, and several update/delete RPCs have no documented public mapping. **Unblock:** an API/Space owner freezes one route table and maps each entry to the canonical Space RPC and ACL before expanding or changing Gateway/Flutter routes and their contract tests; transfer must consume the separately unresolved resource-bound Auth proof. Sources: [api-gateway.md](../microservices/api-gateway.md) § «Канонический формат клиентских API-доков»; [spaces.md](../features/spaces.md) §§ «Инвайт-ссылки», «Аудит-лог», «Pin элемента дерева», «Удаление Спейса»; [space-service.md](../microservices/space-service.md) § Implementation status; `protos/voice/space/v1/space.proto`; `src/backend/gateway/transcode_spaces_invites.go`; `transcode_spaces_tree.go`.
...
- [ ] **[A2 contract / Voice] Implement the recorded self and moderator room-move contract** — `MoveToVoiceRoom` remains the authenticated actor’s self-move; add `MoveParticipantToVoiceRoom` with an explicit target. For a moderator move, the actor needs `VOICE_MOVE_OTHERS` in the source, the target needs `VOICE_JOIN` in the destination, and the actor does not need destination join permission. Actor, target, source, and destination must share a Space; failure leaves both rosters unchanged. **Unblock:** align proto/Gateway/Voice/Role tests and atomic roster/session behavior with this recorded decision. Sources: `tmp/fleet/slave-driver/a2-20260907/CURRENT_STATE.md` § Recorded product and technical decisions; [voice-chat.md](../features/voice-chat.md); [role-service.md](../microservices/role-service.md).
- [ ] **[A2 contract / Space↔Voice] `ResolveVoiceRoomAccess` has a narrow S2S credential but no documented credential lifecycle** — current proto/code fixes the caller as the verified `voice` service: Voice sends exactly `Authorization: Bearer $SPACE_VOICE_S2S_TOKEN`; Space’s interceptor accepts that credential only for this RPC, derives the server-side `voice` identity, and ignores forwarded profile metadata. No canonical deployment/operations document owns issuance, injection into both services, rotation, revocation, or the rollout invariant for this shared value. **Unblock:** record that lifecycle and its fail-closed rollout/rollback rule; do not widen the RPC or infer identity from user metadata. Sources: `protos/voice/space/v1/space.proto` (`ResolveVoiceRoomAccess` comments); `src/backend/voice/internal/s2s/voice_room_access.go`; `src/backend/space/internal/authctx/authctx.go`; `docs/DEPLOYMENT.md` (no resolver credential record).
- [ ] **[A2 contract / Gateway↔Voice] Canonical resolver failures still lack a documented public disclosure contract** — current code preserves direct gRPC `NOT_FOUND` for an absent room, uses `PERMISSION_DENIED` for inactive/non-member/invalid canonical evidence and assertion mismatch, and preserves `UNAVAILABLE` for the resolver; Gateway’s generic adapter maps those codes to 404/403/503 and returns the gRPC message. Neither feature nor API documentation says which caller-visible outcomes may reveal room existence, whether malformed resolver evidence is distinguishable, or freezes the REST `error_code`/body for these cases. **Unblock:** an API owner records one direct-gRPC and REST disclosure mapping, then tests every listed outcome through both paths with no token, session, roster, or event side effect. Sources: `src/backend/voice/internal/grpcsvc/voice_room_access.go`; `voice_room.go`; `src/backend/gateway/transcode_grpc.go`; `tmp/fleet/slave-driver/a2-20260907/A2-P2-acl-design.md` ACL-01–04.
- [ ] **[A2 contract / Gateway↔Voice] Space voice-room lifecycle route contract is not frozen** — the documented Gateway table covers only DM/group `VoiceService` calls; it does not define a public contract for persistent Space-room join, leave, roster, self move, or the recorded moderator move. Existing Gateway code and tests happen to expose and characterize join/leave/states routes and forward a client `space.id` assertion to `JoinVoiceRoom`; they are not a canonical public API. The feature canon and Phase-0 resolver decision do not specify whether that assertion belongs in the public body, nor define route shapes, request/response JSON, auth, roster visibility/redaction, error/disclosure, or event/REST interaction. `MoveParticipantToVoiceRoom` is not yet a proto RPC. **Unblock:** the API/Voice/Space/Role owners freeze those public mappings after the resolver disclosure, roster-visibility, and moderator-move contracts are decided, before expanding or changing routes and their contract or Flutter tests. Sources: [api-gateway.md](../microservices/api-gateway.md) § Voice Service + canonical client API format; [voice-chat.md](../features/voice-chat.md) §§ «UX и статусы», «Спейс»; `protos/voice/calls/v1/calls.proto`; `src/backend/gateway/transcode_voice_rooms.go`; `tmp/fleet/slave-driver/a2-20260907/CURRENT_STATE.md` § Recorded product and technical decisions.
- [ ] **[A2 follow-on / Voice↔LiveKit] Canonical media-grant and revocation semantics remain uncontracted** — `GetJoinToken` re-resolves Space-room access and derives Space-room `video.canPublish` from `VOICE_SPEAK`; the issuer supplies a JWT expiry. The product and service documents do not define signed claim/session binding beyond the LiveKit room and profile identity, token lifetime policy, reissue/token-epoch behavior, or what disconnect/revoke outcome is required after membership, `VOICE_JOIN`, or `VOICE_SPEAK` loss. **Unblock:** a shared Voice/LiveKit contract fixes those fields and the bounded revocation guarantee before implementation claims reconnect, active-publish, or forced-removal coverage. Sources: `src/backend/voice/internal/grpcsvc/voice_grpc.go` (`GetJoinToken`); `src/backend/voice/internal/livekit/token.go`; `docs/features/voice-chat.md`; `docs/microservices/voice-service.md`; `tmp/fleet/slave-driver/a2-20260907/A2-P2-acl-design.md` § Required contract changes 6.
- [ ] **[A2 follow-on / Realtime↔Role] Roster permission is named, but snapshot and fan-out semantics are not** — `SPACE_VIEW_MEMBER_LIST` already canonically means viewing a Space member roster, while the feature requires seeing room occupants before joining. The documents do not say whether a Space member without that permission receives denial or a redacted roster, what fields may be exposed, or how the same rule applies to `GetVoiceStates`, initial snapshots, and room-watcher events. Current `GetVoiceStates` checks only Space membership, and Realtime has no documented Voice-room watcher contract. **Unblock:** Role/Realtime owner records those semantics and applies one rule to snapshot and event delivery. Sources: [role-service.md](../microservices/role-service.md) `SPACE_VIEW_MEMBER_LIST`; [voice-chat.md](../features/voice-chat.md) § UX и статусы; `src/backend/voice/internal/grpcsvc/voice_grpc.go` (`GetVoiceStates`); `tmp/fleet/slave-driver/a2-20260907/A2-P2-acl-design.md` ACL matrix (roster).
...
- [ ] **[A2 contract / Realtime↔Voice] Voice WS event audience and disclosure rules are incomplete** — the Realtime wire table fixes the names and fields of DM `call_*` operations, but does not define the recipient set for every event (including every active tab), payload redaction for room-state/screen-share events, or an ordering/idempotency rule between Voice JetStream events and reconnect. Existing code routes DM payloads to IDs embedded by Voice; the roster-specific snapshot/fan-out/redaction decision is tracked separately above. **Unblock:** product/Voice/Realtime owner records per-operation audience, fields visible to each audience, ordering key and reconnect behavior before extending Voice event fan-out. Sources: [realtime-service.md](../microservices/realtime-service.md) § «Операции (Server → Client)», [voice-chat.md](../features/voice-chat.md), `src/backend/realtime/voice_events_consumer.go`, `protos/voice/events/v1/jetstream_events.proto`.
...
- [ ] **[Space] Audit writers are incomplete for explicitly listed actions** — `CreateInvite` persists an invite without any audit row; tree CRUD, space settings and role changes also have no audit rows. The feature canon requires creation and revocation of invite links plus the listed administrative changes, but it does not freeze an action enum/name, target/details payload, retention, or writer ownership. **Unblock:** Space/product owner defines that audit-event contract before writers and positive audit-entry tests are implemented. Sources: [spaces.md](../features/spaces.md) § «Аудит-лог»; `src/backend/space/internal/store/invite.go`; `src/backend/space/internal/store/tree.go`; `src/backend/space/internal/grpcsvc/invites.go`.
...
- [x] **[Analytics] `role_events` stream consumed** — **done:** the current runner registers `role_events` / `role.>` with `handleRoleMsg`; PR #238 adds bounded retry for missing-stream startup, covered by helper-classification tests (`src/backend/analytics/internal/consumer/runner.go`, `runner_test.go`).
...
- [ ] **[Role/Space] Owner-role mutation versus ownership-transfer authorization is unspecified.** Ordinary member-role RPCs must deny granting, revoking, or reassigning `Owner`; however, the existing compensated `Space.TransferOwnership` flow calls the public Role `AssignRole`/`RevokeRole` RPCs to make the corresponding Owner transition. Before adding a Role guard, bypass, or behavior tests, decide and document the narrow Space→Role contract: authenticated service principal and allowed action; how the request is bound to the exact transfer/space/current-owner/new-owner transaction; idempotency, retry and error mapping; and which service owns each atomic/reversible compensation step. The resolved contract must preserve a dedicated compensated ownership transfer while preventing ordinary member-role calls from changing Owner. — `src/backend/space/internal/grpcsvc/space.go`; `src/backend/space/internal/grpcsvc/roles.go`; `src/backend/role/internal/grpcsvc/roles.go`; `src/backend/role/internal/store/roles.go`; `docs/microservices/space-service.md`
...
- [ ] **[A2 contract / Role S2S and edge identity] Role has no authenticated caller boundary; the earlier four-RPC inventory is incomplete.** `role/main.go` registers only common gRPC middleware, while `role/internal/authctx.ProfileID` directly accepts `x-voice-profile-id`. No Role credential, verified service principal, caller-to-method allow-list, or credential lifecycle is configured. This leaves three distinct current call classes without a frozen contract: (1) the Gateway invokes the client surface and forwards raw profile metadata; the actor-bound mutations `CreateRole`, `UpdateRole`, `DeleteRole`, `ReorderRoles`, `AssignRole`, `RevokeRole`, `Set/Remove{Chat,VoiceRoom}Override`, and `SetDefaultJoinRole` require a verified user principal rather than a caller-supplied header; (2) Space invokes `BootstrapSpaceRoles`, `GetDefaultJoinRole`, `ListRoles`, `GetMemberRoles`, and `AssignRole`/`RevokeRole` for join/leave and compensated ownership transfer, while Bot invokes `DeleteRolesCreatedByProfile`, `GetMemberRoles`, and `RevokeRole` during uninstall and invokes actor-bound role mutations for its bot actor; (3) Space, Voice, Chat, Messaging, and Bot call `CheckPermission` with an explicit subject, while Gateway currently calls it and `GetEffectivePermissions` as a probe. Read RPC disclosure (`ListRoles`, `GetMemberRoles`, both override reads, and `GetDefaultJoinRole`) also has no caller/subject policy. **Unblock:** Role, Gateway, Space, Bot, Voice, Chat, and Messaging owners must record one authenticated principal/credential and rotation boundary; an exact caller/principal/action matrix (including which S2S callers may supply an explicit decision subject and which lifecycle mutations they may perform); edge user-principal propagation and its binding to request actor; TLS/transport ownership; and failure/error semantics. Do not implement a bearer, forwarded-header bypass, principal/action mapping, TLS choice, or tests that encode any of those undecided semantics. The narrow Space-to-Voice resolver bearer is not reusable precedent: it is endpoint-specific and its credential lifecycle remains separately unresolved. Sources: `src/backend/role/main.go`; `src/backend/role/internal/authctx/authctx.go`; `src/backend/role/internal/grpcsvc/{roles.go,roles_manage.go,roles_cleanup.go}`; `src/backend/gateway/{transcode_grpc.go,transcode_roles.go}`; `src/backend/space/internal/grpcsvc/roles.go`; `src/backend/{voice,messaging}/internal/s2s/role_permissions.go`; `src/backend/chat/internal/grpcsvc/roles.go`; `src/backend/bot/internal/grpcsvc/{bot_c.go,interaction.go}`; `src/backend/space/internal/authctx/authctx.go`; `docs/todo/backend.md` A2 Space-to-Voice credential-lifecycle gap.
...
- [ ] **[Space/Auth] Resource-bound step-up proof for ownership transfer is unspecified** — [spaces.md](../features/spaces.md) requires password/2FA confirmation, but it does not define the Auth proof contract: issuer and consumer, authenticated subject, required factors, expiry, one-time/replay and revocation semantics, or binding to the exact `space_id` and `new_owner_profile_id`. `TransferOwnershipRequest` currently contains only those two identifiers, while Auth exposes only direct account-deletion re-auth. Decide and document that cross-service contract (including fail-closed error mapping) before adding a proto, implementation, or characterization tests; otherwise a proof can be replayed or applied to a different transfer target. — `docs/features/spaces.md`; `docs/microservices/auth-service.md`; `protos/voice/auth/v1/auth.proto`; `protos/voice/space/v1/space.proto`
