...
- [ ] **[Space] Audit writers are incomplete for explicitly listed actions** — `CreateInvite` persists an invite without any audit row (characterized by `TestCreateInvite_DoesNotRecordAuditEntry`); tree CRUD, space settings and role changes also have no audit rows. The feature canon requires creation and revocation of invite links plus the listed administrative changes, but it does not freeze an action enum/name, target/details payload, retention, or writer ownership. **Unblock:** Space/product owner defines that audit-event contract before writers and positive audit-entry tests are implemented. Sources: [spaces.md](../features/spaces.md) § «Аудит-лог»; `src/backend/space/internal/store/invite.go`; `src/backend/space/internal/store/tree.go`; `src/backend/space/internal/grpcsvc/invites.go`.
