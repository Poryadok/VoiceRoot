...
- [ ] **[Social] Flutter client surface incomplete** — `friends_client.dart` now has contacts/favorites (**Batch 23b**), `listBlocked`/`unblockAccount` + Blocked tab (**Batch 24a**); still missing `syncPhoneContacts`. Gateway exposes phone sync (`transcode_friends.go`).
