...
- [x] **[Story] Archive purge worker delayed first run** — **done:** `StartArchivePurgeWorker` runs `RunArchivePurgeOnce` on startup before its daily ticker; the service lifecycle context cancels both workers. `TestStartArchivePurgeWorker_runsOnceOnStartup` verifies immediate media deletion, row cleanup and idempotent later execution.
