...
- [ ] **[User] R3-A19 — Presence WS privacy filter (code)** — Realtime `presence_update` must apply `show_online` / omit `last_seen` per viewer — User read-path filtering is shipped, but Realtime fan-out remains open; doc in [presence.md](../features/presence.md).
...
- [ ] **[Story] `media_file_id` has no exact Story-context attestation** — Story now fails closed when File metadata is unavailable and validates uploader ownership, ready/clean lifecycle, chat-context exclusion, photo/video type and video duration. The remaining canonical gap is specific: `RequestUpload.context_story` is only stored in File; `FileMetadata` and File reads do not return it, `GetFileMetadata` is gateway/profile scoped rather than a protected Story S2S attestation, and Story obtains its ID only during CreateStory after the Flutter upload. Define a preallocated Story ID/draft flow plus a protected File `ValidateStoryMedia`-style exact binding contract before accepting an upload as bound to a particular Story. Paths: `protos/voice/file/v1/file.proto`; `src/backend/file/internal/grpcsvc/file_grpc.go`; `src/backend/story/internal/grpcsvc/story.go`; `src/frontend/lib/ui/stories/story_create_screen.dart`.
