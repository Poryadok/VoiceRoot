...
- [x] **[Cross-cutting] Device `integration_test` driver suite — partial:** host scaffold `integration_test/device_driver_smoke_test.dart` + CI job `flutter-device-driver` (deeplink + FCM/VoIP register contracts). Still open: physical-device alert delivery (NT-05), App Links/AASA, CallKit/PushKit/LiveKit media. — `src/frontend/integration_test/README.md`, `docs/TESTING.md`
