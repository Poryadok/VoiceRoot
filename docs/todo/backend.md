...
- [ ] **[Messaging / C-002, P-008] `message.sent` event** — part of the lifecycle vertical above. **Partial (parallel track):** `MessageSent.content_type` shipped; PR #310 owns `send_silent = 8`, then P-008 adds only `was_scheduled = 9` and optional `scheduled_at = 10`.
