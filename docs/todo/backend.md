...
- [ ] **[A2 Gateway/Space] Implement frozen public lifecycle, invite, tree and audit contract** — [API Gateway A2 target](../microservices/api-gateway.md#a2-space-rest-contract-target) now defines routes, JSON, ACL/disclosure, replay and paging. Remaining: Space proto/service operation fields and durable journal/outcomes, wiring the shipped Auth proof consume/receipt path into Space transfer, separate deletion-proof lifecycle, signed caller cutover, audit outbox/filter cursor, Gateway transcoders and Flutter vertical with negative transport tests. Existing selected routes remain characterization only; expose target behavior only after the complete corresponding vertical is verified.
