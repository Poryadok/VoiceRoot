...
- [ ] **[Space] Owner locked: `GetAuditLog`, `SearchPublicSpaces`, `ListTemplates`, `CreateFromTemplate` — runtime `Unimplemented` (embed). `DeleteSpace`/`TransferOwnership`/`JoinSpace`/`LeaveSpace` живые; owner не может leave без transfer** — `protos/voice/space/v1/space.proto`; `src/backend/space/internal/grpcsvc/`
