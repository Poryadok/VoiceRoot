...
- [ ] **[Subscription] CloudPayments lifecycle incomplete — protected Space Pro activation exists, but personal activation, renew/failure/recovery/cancel/resume, normalized provider ordering and the Gateway webhook route are absent.** — `src/backend/subscription/internal/grpcsvc/subscription.go` (`HandleCloudPaymentsWebhook`); `src/backend/gateway/transcode_subscription.go`; target state machine: [subscription lifecycle RED plan](../testing/subscription-lifecycle-convergence-exec-plan.md)
- [ ] **[Subscription/Auth] JWT tier projection is not durable/replay-safe — Auth binds the NATS store only with `AUTH_NATS_URL`, otherwise uses memory; the consumer applies arrival order in memory and cannot bootstrap after stream MaxAge. JWT also lacks entitlement revision/deadline, so an issued Premium claim can outlive the authoritative boundary. Implement the accepted inbox/snapshot plus `subscription_revision`/`subscription_entitled_until` contract.** — `NatsSubscriptionTierStore.java`; `JwtService.java`; [subscription lifecycle RED plan](../testing/subscription-lifecycle-convergence-exec-plan.md). **ops:** `AUTH_NATS_URL` on staging/prod — [ci.md](ci.md) § Critical.
...
- [ ] **[Cross-cutting] Implement accepted `subscription.entitlement_changed` convergence — producer UUIDs are allocated after the state commit with no transactional outbox/`Nats-Msg-Id`; Auth/Space/Voice/Notification lack durable inbox/revision handling, User lacks a lifecycle consumer, File ACKs only into an unwired cache, Analytics mapping/cutover is partial, and no enforcement consumer can safely bootstrap after MaxAge.** — [canonical contract and RED matrix](../testing/subscription-lifecycle-convergence-exec-plan.md); `src/backend/subscription/internal/subscriptionevents/jetstream.go`; `docs/CONTRACT_MATRIX.md`
...
- [x] **[Subscription] Basic `subscription.events` producer wiring — direct publisher emits personal activation/payment/expiry/downgrade/reminder events.** — `src/backend/subscription/internal/subscriptionevents/jetstream.go`; `src/backend/subscription/main.go`; `src/backend/subscription/internal/sweeper/sweeper.go`. **Not durable:** stable transactional outbox/revisioned authoritative event remains in the cross-cutting lifecycle item.
...
- [ ] **[Subscription] Paddle lifecycle is non-convergent — handlers exist for activation/failure/renew/cancel/pause, but event arrival is treated as order, normalized provider metadata/version is incomplete, renew emits no authoritative domain snapshot, and state commit can lose its post-commit publish.** — `src/backend/subscription/internal/grpcsvc/subscription.go`; accepted state machine/outbox: [subscription lifecycle RED plan](../testing/subscription-lifecycle-convergence-exec-plan.md)
- [ ] **[Subscription/User/Flutter] Implement accepted downgrade selection/unfreeze — current manual RPC is not bound to entitlement state/revision, does not enforce two distinct owned IDs transactionally, lazy switch freezes an arbitrary pair, and renewal cannot unfreeze subscription-owned profiles.** — `src/backend/subscription/internal/grpcsvc/user_profile_downgrade.go`; `src/backend/user/internal/grpcsvc/user_verification.go`; `src/frontend/lib/ui/profile/profile_downgrade_picker_screen.dart`; [target](../testing/subscription-lifecycle-convergence-exec-plan.md)
- [ ] **[Subscription/Space] Implement accepted Space Pro payment-failure grace — use the same revisioned P7D state machine and D1/D3/D7 schedule as personal Premium; preserve Pro caps through grace, recover on payment and remove only growth entitlement at final expiry.** — `src/backend/subscription/internal/sweeper/sweeper.go`; `src/backend/subscription/internal/store/sweeper.go`; [target](../testing/subscription-lifecycle-convergence-exec-plan.md)
- [ ] **[Subscription/Account deletion] Subscription is absent from delete/restore/purge coordination — implement immediate personal inactive, durable provider auto-renew cancel, explicit restore without implicit billing resume, Space purchaser cancellation semantics, participant purge receipts, raw-ID crypto-erasure and permanent HMAC replay tombstone.** — [accepted lifecycle RED plan](../testing/subscription-lifecycle-convergence-exec-plan.md); account deletion A4 in [PLAN.md](../PLAN.md)
- [ ] **[Subscription/File] Implement accepted account/reference retention recalc — File stores only uploader profile; its legacy subscription consumer ACKs into an unwired cache and never changes retention, so free→Premium deadlines remain and Premium→free rows can remain forever. Add immutable `retention_account_id` to each exact reference, authorized owner backfill, downgrade+P90D recalc, renewal rescue only before expiry, and no expired/GC resurrection; E2E remains upload+P90D.** — `src/backend/file/main.go`; `src/backend/file/internal/grpcsvc/file_grpc.go`; `src/backend/file/internal/store/files_store.go`; [target](../testing/subscription-lifecycle-convergence-exec-plan.md)
...
- [x] **[Story/File] Protected Story-media admission** — `ValidateStoryMedia` uses File-owned locked-row validation and a strict `service:story` TLS/JWKS/shared-Redis replay boundary. Story signs exact photo/video requests and maps dependency failures to `Unavailable`; it never forwards Gateway identity or calls `GetFileMetadata` for admission. Predicate, PostgreSQL lock-order, signed transport and Story no-write/no-event matrices cover the path. Deployment configuration is explicit in [story-service.md](../microservices/story-service.md#protected-media-rollout).
- [ ] **[Story/File] LFP media-kind policy and exact Story reference lifecycle** — media-bearing LFP remains `FailedPrecondition` until a product enum/duration policy exists. Read-only admission does not claim an exact Story-ID reference; acquire/release, retry ownership and reference-safe deletion remain separate lifecycle work. Paths: `protos/voice/file/v1/file.proto`, `src/backend/file/internal/grpcsvc/`, `src/backend/story/internal/grpcsvc/`.
...
- [ ] **[Subscription/Notification] Implement accepted grace reminder schedule/delivery — replace publish-before-mark arrays with revision-bound D1/D3/D7 scheduled outbox rows and fenced stable publish; Notification needs inbox, unique in-app row and leased push outbox. Push is at-least-once with a stable logical/client collapse ID, not impossible provider exactly-once. Route to payer/purchaser primary profile through push + in-app; email remains auth-only.** — `src/backend/subscription/internal/sweeper/sweeper.go`; `src/backend/subscription/internal/store/grace_reminders.go`; `src/backend/notification/subscription_events_consumer.go`; [target](../testing/subscription-lifecycle-convergence-exec-plan.md)
...
- [x] **[File/Gateway/Flutter] Thumbnail URL variant (P-007)** — `UNSPECIFIED` preserves converted→original selection; `THUMBNAIL` presigns only `thumbnail_r2_key` after the same File ACL/surface checks, with no fallback. Gateway accepts only `?variant=thumbnail`; Flutter resolves image previews through that presigned URL and refreshes the same variant after `403`/`410`. Keys and direct object URLs are not public API. Canon: [file-service.md](../microservices/file-service.md#thumbnail-url-variant), [api-gateway.md](../microservices/api-gateway.md), [file-storage.md](../features/file-storage.md).
...
- [ ] **[Analytics] Search/Voice dashboard Query API contract and types missing** — Only `product`, `engagement`, `revenue`, `health`, `moderation` are supported. Search/Voice labels are not implementable safely yet: formulas/point shape/filter and range semantics are undefined; Search identity/RPC coverage, zero/click-position facts and publish-loss policy are missing; Voice lacks canonical concurrency/duration mapping plus negotiated-codec authority. The required decisions and RED evidence are frozen in [analytics-search-voice-dashboard-contract.md](../testing/analytics-search-voice-dashboard-contract.md). Federation remains deferred and is not part of this slice (`src/backend/analytics/internal/store/query.go`, `docs/microservices/analytics-service.md`).
...
- [ ] **[Subscription] E2E / test gaps — no fake/sandbox lifecycle proof for commit→publish crash, provider replay/reverse order, consumer restart/snapshot, reminder claim/dedupe, File recalc/renewal, User unfreeze, Space Pro failed-payment grace, provider-side cancel or billing history.** Personal grace expiry has narrow service coverage; Space Pro activation→join is `TestComposeSpaceProMemberCap_live`. — [RED matrix](../testing/subscription-lifecycle-convergence-exec-plan.md); `src/frontend/test/billing_e2e_live_test.dart`; `src/backend/gateway/compose_billing_live_test.go`
...
- [ ] **[Subscription/Space H1] Space ownership transfer billing semantics are undefined — canon says the owner pays for Space Pro but does not say whether the old purchaser continues, renewal cancels, or a new owner assumes a cycle. Payment credentials must never transfer by inference; decide before enabling the ownership-transfer + live-billing combination.** — `docs/features/subscription.md`; `docs/features/spaces.md`; [lifecycle risks](../testing/subscription-lifecycle-convergence-exec-plan.md)
