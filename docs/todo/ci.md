...
- [ ] **[A1] Clean-install Voice on staging and verify live messaging** — The initial authorized install attempt was [36064271554](https://github.com/Poryadok/VoiceRoot/actions/runs/36064271554) at `b0f22dfc98b9a60aed584fb686913ab7f1a655aa`; it wiped only `voice-staging` and Voice-owned PVCs, while the k3s node and other namespaces remained intact. Deployment then failed at `voice-search`: JetStream was disabled through the app-local NATS leaf, so smoke was skipped and application readiness was incomplete. No further namespace or database wipe is authorized; PR [#481](https://github.com/Poryadok/VoiceRoot/pull/481) removes the deploy-time wipe path and prevents stale reset markers from changing the live NATS Service route. Current PR head is `79f2af8e42d550aef95b87d50974d26580be2685` on base `ec647bc9cf1f8eb7da7dd6d78d26d144fdd3deb9`; it is not merged and has no GitHub approval. Hosted CI run [36184261281](https://github.com/Poryadok/VoiceRoot/actions/runs/36184261281) completed with 52 passing, 16 skipped, and one failure: `a1-attachment-restart-proof` cannot anonymously pull the pinned Quay MinIO image (`unauthorized`). No identical anonymous digest was confirmed on Docker Hub or GHCR. After registry access is restored and PR checks/review pass, merge and verify the master SHA; run the read-only NATS diagnostic; then deploy with data-preserving settings and verify NATS durables/permissions, all application readiness, staging HTTP smoke, and two-user A1 messaging E2E per [`PLAN.md`](../PLAN.md).
