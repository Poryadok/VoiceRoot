...
### [A1] Staging clean install and acceptance

- [ ] **[A1] Clean-install Voice on staging and verify live messaging** — The initial authorized install attempt was [36064271554](https://github.com/Poryadok/VoiceRoot/actions/runs/36064271554) at `b0f22dfc98b9a60aed584fb686913ab7f1a655aa`; it wiped only `voice-staging` and Voice-owned PVCs, while the k3s node and other namespaces remained intact. Deployment then failed at `voice-search`: JetStream was disabled through the app-local NATS leaf, so application readiness was incomplete. No further namespace or database wipe is authorized. PR [#481](https://github.com/Poryadok/VoiceRoot/pull/481) merged as `86b2017f06d0d471e8b43abc78031e86756defe3`; it removes the deploy-time wipe path and prevents stale reset markers from changing the live NATS Service route. Its exact-head CI passed (54 checks, zero failures), and the read-only image publishers produced verified GHCR digests; staging-only pins are tracked separately. Diagnostic run [36194770945](https://github.com/Poryadok/VoiceRoot/actions/runs/36194770945) did not reach kubectl or NATS: the staging runner could not connect to `github.com:443` during checkout, and all probe/configuration steps were skipped. The automatic `STAGING_DEPLOY_ENABLED` Actions variable is temporarily `false`; manual workflow dispatch remains available. Restore the automatic switch only after the NATS cause is fixed and full staging acceptance passes. Next restore runner GitHub connectivity, rerun the read-only diagnostic, then deploy with data-preserving settings and verify NATS durables/permissions, all application readiness, staging HTTP smoke, and two-user A1 messaging E2E per [`PLAN.md`](../PLAN.md).
...
- [ ] **Messaging → Moderation staging smoke** — ConfigMap wiring is shipped: `MODERATION_GRPC_ADDR=voice-moderation:9090` is present in both staging/prod shared ConfigMaps and Messaging consumes them through `envFrom`. Still capture live staging evidence that the Messaging send path enforces Moderation shadow-ban and spam-mute checks; the existing smoke covers Gateway → Moderation admin calls, not this Messaging S2S path. DoD: a staging smoke proves both enforcement cases after deployment.
