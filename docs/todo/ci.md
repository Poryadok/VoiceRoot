...
- [ ] **[A1] Clean-install Voice on staging and verify live messaging** — PR [#479](https://github.com/Poryadok/VoiceRoot/pull/479) merged as `b0f22dfc98b9a60aed584fb686913ab7f1a655aa`; hosted staging run [36064271554](https://github.com/Poryadok/VoiceRoot/actions/runs/36064271554) used that SHA. `STAGING_NATS_SECRETS_B64` exists in the `staging` Environment by name, and its restore step passed; its value was never read or exposed. User explicitly authorized erasing all Voice data and reinstalling from scratch; the run deleted/recreated only the dedicated `voice-staging` namespace and Voice-owned PVCs. The k3s node remained Ready and `voice-observability`, `penpot-staging`, and all other namespaces remained Active. Image verification, NATS storage preflight, all secret restores, four NATS bootstrap Jobs, and seven DB migration Jobs passed. `Apply staging manifests` failed waiting for `voice-search`; hosted diagnostics report `nats: jetstream not enabled` via the app-local NATS leaf and a refused connection to `voice-nats:7422`. The smoke step was skipped and multiple application deployments remain unready. The staging NATS leaf/service manifest path is the next blocker; do not rerun unchanged. Current task scope prohibits changing application code or manifests, so await explicit scope approval before attempting that fix. Keep the regular state-preserving NATS migration gate for ordinary full deploys. After a successful deployment require all 40 fixed durables, NATS allowed publish/consume/ACK and neighboring-subject denial, all application rollout/readiness checks, staging HTTP smoke, and two-user A1 messaging E2E per [`PLAN.md`](../PLAN.md).
