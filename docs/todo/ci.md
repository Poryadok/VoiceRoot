...
- [ ] **[A1] Clean-install Voice on staging and verify live messaging** — hosted `Staging deploy` run [36040281106](https://github.com/Poryadok/VoiceRoot/actions/runs/36040281106) used master SHA `56daf693d217329462ab1edba487ae9a2b932208`; image checks and `Restore NATS activation secrets` passed, but `Apply staging manifests` stopped because `voice-nats-realtime-bootstrap` was incomplete. User explicitly authorizes erasing all Voice data and installing from scratch; the wipe boundary is only the dedicated `voice-staging` namespace and its Voice-owned PVCs. Preserve the Kubernetes cluster, node/server, `voice-observability`, `penpot-staging`, and every other namespace. The manual-only clean-install option in [`Staging deploy`](../../.github/workflows/staging-deploy.yml) must verify the target image, NATS storage class, and operator-confirmed capacity before deleting/recreating only `voice-staging`, restore its secrets, start the new PVC-backed NATS hub, run all four bootstrap Jobs, then deploy the exact integrated master SHA and HTTP smoke. Keep the regular state-preserving NATS migration gate for normal full deploys. Confirm `STAGING_NATS_SECRETS_B64` by name/restore status only; never print or decode its value. After deployment require all 40 fixed durables, NATS allowed publish/consume/ACK and neighboring-subject denial, all application rollout/readiness checks, staging HTTP smoke, and two-user A1 messaging E2E per [`PLAN.md`](../PLAN.md). The old failed run is not proof of deployment; run clean install only after image and storage preflight pass.
