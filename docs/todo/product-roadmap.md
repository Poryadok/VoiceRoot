...
- [ ] **Спека vs код (решения человека)** — стикеры в [text-chat.md](../features/text-chat.md) при 0 кода: v1 или вычеркнуть; папка DM requests в спеке vs PLAN «не MVP»; user-add игр vs staff `CreateGame`; space-MM уже в коде (П.1) — зафиксировать в спеке как v1. **Не править PLAN.md в этом прогоне.**
