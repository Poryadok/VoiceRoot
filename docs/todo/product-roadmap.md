...
- [x] **П.10 — Уведомления: тихие часы, гранулярность, voice join** — Set/Get quiet hours lives (`TestComposeQuietHours_live`, `quiet_hours_e2e_live_test`, P2.5); message-family FCM/APNs payloads use one atomic collapse sequence per recipient/chat, while `message_request` groups by sender; presence routing keeps in-app delivery when push is suppressed and preserves the documented MM/voice exceptions. Отдельный `reply` wire fan-out остаётся в [backend TODO](backend.md). Спека: [notifications.md](../features/notifications.md).
...
- [ ] **П.15 — Бот-платформа v2 (partial)** — **shipped:** Flutter `/` slash picker, subcommands UI, Developer Portal catalog basics, deferred interaction ChatRef preservation. **Open:** inbound `message.events` → webhook consumer; Portal CSRF/manifest gaps; token rotation hub purge. Спека: [bots.md](../features/bots.md). → follow-up Bot batches.
