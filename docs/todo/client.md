...
- [ ] **[A2 Voice] Flutter voice-room leave lifecycle is not wired** — `VoiceCallsClient.leaveVoiceRoom` exposes the current characterized `POST /api/v1/voice/rooms/{voiceRoomId}/leave` route, but `CallController.joinVoiceRoom` discards `VoiceRoomSession.voiceRoomId`; its generic `hangUp` therefore calls `/voice/calls/{roomId}/end` instead. Do not add a client fallback or assume equivalence: the public lifecycle route contract itself remains unresolved in [backend.md](backend.md) § A2 contract / Gateway↔Voice. Once it is frozen, retain `voiceRoomId` in the active client session and test the contract-defined leave/disconnect/roster outcomes, including failure and side-effect ordering only where the owner specifies them. Sources: `src/frontend/lib/backend/voice_client.dart`; `src/frontend/lib/state/call_providers.dart`; `tmp/fleet/slave-driver/a2-20260907/A2-P4-flutter-technical-ux.md` §§ Active voice panel, Required provider/client boundary.
