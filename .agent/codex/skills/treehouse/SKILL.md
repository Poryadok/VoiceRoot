...
description: >-
  ...
  status, prune, in-project pool (root=.), Codex workspace alignment. Use when
...
## When To Use Treehouse

| Ситуация | Инструмент |
|----------|------------|
| Терминальный агент, долгоживущий пул, кэш deps | **treehouse** |
| Quick one-off in a Codex-created worktree | Use the explicit Codex worktree/thread mechanism chosen for that task |
...
### Codex workspace = path worktree

**Критично:** если Codex workspace открыт на `D:\Git\Voice`, а терминал в worktree на `C:\Users\...\.treehouse\...`, агент правит **не те файлы**.

Перед работой:

1. Узнать path: вывод `treehouse get` / `treehouse status` / `treehouse get --lease --json`
2. **Workspace / agent root** = этот path (open or hand off the task in that worktree path)

Внутри worktree действуют те же `AGENTS.md`, `.agent/`, `docs/`, TDD — это полная копия репо.
...
| Два параллельных механизма WT | treehouse **или** another worktree mechanism |
...
Параллельные crew в Codex → skill `voice-fleet-captain` (backlog, brief, spawn). Treehouse выдаёт path; captain записывает в `tasks/T-*.meta`.
...
| Тема | Файл / skill |
|------|----------------|
| Fleet captain / backlog | `voice-fleet-captain` |
| Git, PR, no rewrite | `voice-git-workflow` |
