...
1. Read `.agent/codex/skills/penpot-voice/SKILL.md` completely when the task touches Penpot, frames, or MCP.
...
5. **Flutter parity** (only if the user asked to implement UI): `VoiceColors.of(context)` / `Theme.of(context).colorScheme`, widgets in `src/frontend/lib/ui/core/`. No `Color(0x…)`, `Colors.indigo`, or `ColorScheme.fromSeed` in `lib/ui/**` or `lib/shell/**`. Rule: `.agent/codex/rules/voice-design.mdc`.
