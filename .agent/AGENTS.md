...
- For implementation and behavior-changing fixes, use `tdd-code-workflow` within
  the scope in `docs/TESTING.md`. Its canonical source is
  `.agent/workflows/tdd-code-workflow/SKILL.md`; the portable Codex copy is
  `.agent/codex/skills/tdd-code-workflow/SKILL.md`. Cursor loads a stub from
  `.cursor/skills/tdd-code-workflow/SKILL.md`. Development completion and
  verification decisions belong in that skill, not duplicated here.
...
- Treat action requests as authorization to perform the requested work, not
  merely propose it. Preserve earlier authorization; do not ask again for
  read-only investigation or routine, reversible steps within that scope.
- Resolve routine choices from context. Ask a focused question only when missing
  information materially affects correctness, scope, or authorization. Pause
  only dependent work and continue independent authorized work while awaiting input.
- Before a necessary approval request, finish authorized preparation and present
  the concrete result, exact action, and unresolved decision. Obtain any required
  approval before destructive, irreversible, or otherwise unauthorized actions;
  task completion does not grant additional permissions.
- Explicit user instructions override conflicting skill guidelines, subject to
  higher-priority instructions and actual permission boundaries. Do not invent
  approval gates from workflow exceptions.
- If a skill causes a pause or deviation, link the exact `SKILL.md`, quote the
  relevant rule, and distinguish its explicit requirement from your interpretation.
...
- Lead with the result, why it matters, verification, and material limitations.
  ...
  steps or parallel items, without stock phrases or unnecessary headings.
- Report concrete blockers and material uncertainty. Omit boilerplate warnings
  about hypothetical risks and empty risk sections when there is nothing to report.
