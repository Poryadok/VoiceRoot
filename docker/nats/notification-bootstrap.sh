...
stream() {
  name="$1"; shift
  echo "bootstrap notification stream INFO $name" >&2
  expected="$(printf '%s\n' "$@" | jq -R . | jq -sc 'sort')"
  if info="$(nats --server "$nats_url" req --raw "\$JS.API.STREAM.INFO.$name" "" 2>&1)"; then
    if printf '%s' "$info" | jq -e '.error' >/dev/null; then
      ...
      echo "bootstrap notification stream ready $name" >&2
      return
    fi
  else
    echo "$info" >&2; exit 1
...
consumer() {
  ...
  echo "bootstrap notification consumer INFO $stream_name/$durable" >&2
  ...
  echo "bootstrap notification consumer CREATE $stream_name/$durable" >&2
  ...
  echo "bootstrap notification consumer ready $stream_name/$durable" >&2
}
