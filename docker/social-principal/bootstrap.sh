...
umask 077
if [ -f /ca/ready ]; then
  for file in /ca/ca.crt /signing/current.pem /signing/next.pem /social/tls.crt /social/tls.key /user/tls.crt /user/tls.key /space/tls.crt /space/tls.key; do
...
for directory in /ca /signing /social /user /space; do
