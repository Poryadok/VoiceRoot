...
umask 077
if [ -f /ca/ready ]; then
  for file in /ca/ca.crt /signing/current.pem /signing/next.pem /social/tls.crt /social/tls.key /user/tls.crt /user/tls.key /space/tls.crt /space/tls.key /auth/tls.crt /auth/tls.key /moderation-keys/current.pem /moderation-keys/next.pem /moderation/tls.crt /moderation/tls.key; do
...
for directory in /ca /signing /social /user /space /auth /moderation /moderation-keys; do
...
openssl genpkey -algorithm RSA -pkeyopt rsa_keygen_bits:2048 -out /moderation-keys/current.pem 2>/dev/null
openssl genpkey -algorithm RSA -pkeyopt rsa_keygen_bits:2048 -out /moderation-keys/next.pem 2>/dev/null
...
for service in social user space auth moderation; do
  ...
  printf 'basicConstraints=critical,CA:FALSE\nkeyUsage=critical,digitalSignature,keyEncipherment\nsubjectAltName=DNS:%s,DNS:voice-%s\nextendedKeyUsage=serverAuth\n' "$service" "$service" > "/tmp/$service.ext"
...
rm -f /tmp/ca.key
chown -R 65532:65532 /signing /social /user /space /auth /moderation /moderation-keys
chmod 700 /signing /social /user /space /auth /moderation /moderation-keys
chmod 644 /ca/ca.crt
touch /ca/ready
