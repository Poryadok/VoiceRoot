...
umask 077
if [ -f /ca/ready ]; then
  for file in /ca/ca.crt /signing/current.pem /signing/next.pem /file-signing/current.pem /file-signing/next.pem /social/tls.crt /social/tls.key /user/tls.crt /user/tls.key /space/tls.crt /space/tls.key /file/tls.crt /file/tls.key; do
...
for directory in /ca /signing /file-signing /social /user /space /file; do
...
for service in social user space file; do
...
rm -f /tmp/ca.key
chown -R 65532:65532 /signing /file-signing /social /user /space /file
chmod 700 /signing /file-signing /social /user /space /file
chmod 644 /ca/ca.crt
touch /ca/ready
