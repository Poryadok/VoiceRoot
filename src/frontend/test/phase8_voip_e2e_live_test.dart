...
///
/// ```text
/// flutter test test/phase8_voip_e2e_live_test.dart ^
///   --dart-define=VOICE_RUN_LIVE_INTEGRATION=true ^
///   --dart-define=VOICE_API_BASE_URL=http://127.0.0.1:18080
/// ```
void main() {
  test(
    'register-device accepts voip_apns token for authenticated user',
    () async {
      ...
      final user = await ctx.registerUser('voip-phase8');
      ...
      final result = await notifications.registerDevice(
        ...
        platform: 'ios',
        token:
            'qa-voip-${user.activeProfileId}-${DateTime.now().millisecondsSinceEpoch}',
