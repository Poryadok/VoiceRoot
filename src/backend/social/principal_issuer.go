...
const (
	socialPrincipalSigningKeysDirEnv = "SOCIAL_PRINCIPAL_SIGNING_KEYS_DIR"
	socialPrincipalActiveKIDEnv      = "SOCIAL_PRINCIPAL_ACTIVE_KID"
)
...
// loadSocialPrincipalIssuerFromEnv keeps the Phase-0 signer disabled until both
...
func loadSocialPrincipalIssuerFromEnv() (*principal.Issuer, principalJWKS, error) {
	if _, set := os.LookupEnv("S2S_SIGNING_KEY_PEM"); set {
		return nil, principalJWKS{}, errors.New("S2S_SIGNING_KEY_PEM is not supported; configure Social principal key directory")
	}
	if _, set := os.LookupEnv("S2S_SIGNING_KID"); set {
		return nil, principalJWKS{}, errors.New("S2S_SIGNING_KID is not supported; configure SOCIAL_PRINCIPAL_ACTIVE_KID")
	}
	dir, dirSet := os.LookupEnv(socialPrincipalSigningKeysDirEnv)
	kid, kidSet := os.LookupEnv(socialPrincipalActiveKIDEnv)
	dir, kid = strings.TrimSpace(dir), strings.TrimSpace(kid)
	if !dirSet && !kidSet {
		return nil, principalJWKS{}, nil
	}
	if dir == "" || kid == "" {
		return nil, principalJWKS{}, fmt.Errorf("%s and %s must both be configured", socialPrincipalSigningKeysDirEnv, socialPrincipalActiveKIDEnv)
	}
	if !validPrincipalKeyID(kid) {
		return nil, principalJWKS{}, fmt.Errorf("%s is invalid", socialPrincipalActiveKIDEnv)
	}
	...
	issuer, err := principal.NewIssuer(principal.IssuerConfig{Issuer: "social", KeyID: kid, PrivateKey: current})
	if err != nil {
