...
const (
	spacePrincipalSigningKeysDirEnv = "SPACE_PRINCIPAL_SIGNING_KEYS_DIR"
	spacePrincipalActiveKIDEnv      = "SPACE_PRINCIPAL_ACTIVE_KID"
)
...
// loadSpacePrincipalIssuerFromEnv keeps the Phase-0 signer disabled until both
...
func loadSpacePrincipalIssuerFromEnv() (*principal.Issuer, principalJWKS, error) {
	if _, set := os.LookupEnv("S2S_SIGNING_KEY_PEM"); set {
		return nil, principalJWKS{}, errors.New("S2S_SIGNING_KEY_PEM is not supported; configure Space principal key directory")
	}
	if _, set := os.LookupEnv("S2S_SIGNING_KID"); set {
		return nil, principalJWKS{}, errors.New("S2S_SIGNING_KID is not supported; configure SPACE_PRINCIPAL_ACTIVE_KID")
	}
	dir, dirSet := os.LookupEnv(spacePrincipalSigningKeysDirEnv)
	kid, kidSet := os.LookupEnv(spacePrincipalActiveKIDEnv)
	dir, kid = strings.TrimSpace(dir), strings.TrimSpace(kid)
	if !dirSet && !kidSet {
		return nil, principalJWKS{}, nil
	}
	if dir == "" || kid == "" {
		return nil, principalJWKS{}, fmt.Errorf("%s and %s must both be configured", spacePrincipalSigningKeysDirEnv, spacePrincipalActiveKIDEnv)
	}
	if !validPrincipalKeyID(kid) {
		return nil, principalJWKS{}, fmt.Errorf("%s is invalid", spacePrincipalActiveKIDEnv)
	}
	...
	issuer, err := principal.NewIssuer(principal.IssuerConfig{Issuer: "space", KeyID: kid, PrivateKey: current})
	if err != nil {
