package matcher_test

import (
	"context"
	"testing"
	"time"

	"github.com/alicebob/miniredis/v2"
	"github.com/google/uuid"
	"github.com/redis/go-redis/v9"
	"github.com/stretchr/testify/require"

	"voice/backend/matchmaking/internal/criteria"
	"voice/backend/matchmaking/internal/matcher"
	"voice/backend/matchmaking/internal/queue"
	"voice/backend/matchmaking/internal/store"
)

func TestWorker_RunOncePaginatesActiveGames(t *testing.T) {
	if testing.Short() {
		t.Skip()
	}

	ctx := context.Background()
	pool := store.StartMatchmakingDBForStoreTest(t, ctx)
	store.ApplyMatchmakingMigrationsForStoreTest(t, ctx, pool)

	games := &store.GameStore{Pool: pool}
	target, err := games.Create(ctx, "MM Pagination Target", duoGameConfig(), uuid.New())
	require.NoError(t, err)
	for i := 0; i < 101; i++ {
		_, err := games.Create(ctx, "MM Pagination Newer "+uuid.NewString(), duoGameConfig(), uuid.New())
		require.NoError(t, err)
	}
	// Make the ordering independent of timestamp resolution and insert timing.
	_, err = pool.Exec(ctx, `UPDATE games SET created_at = now() - interval '1 hour' WHERE id = $1`, target.ID)
	require.NoError(t, err)

	sessions := &store.SessionStore{Pool: pool}
	matches := &store.MatchStore{Pool: pool}
	crit := criteria.MustMarshal(criteria.SearchCriteria{Region: "eu"})
	timeout := time.Now().UTC().Add(30 * time.Minute)
	sessA, err := sessions.Create(ctx, store.CreateSessionParams{
		ProfileID: uuid.New(),
		GameID:    target.ID,
		Mode:      "Duo",
		Criteria:  crit,
		TimeoutAt: timeout,
	})
	require.NoError(t, err)
	sessB, err := sessions.Create(ctx, store.CreateSessionParams{
		ProfileID: uuid.New(),
		GameID:    target.ID,
		Mode:      "Duo",
		Criteria:  crit,
		TimeoutAt: timeout,
	})
	require.NoError(t, err)

	mr := miniredis.RunT(t)
	rdb := redis.NewClient(&redis.Options{Addr: mr.Addr()})
	t.Cleanup(func() {
		_ = rdb.Close()
		mr.Close()
	})
	q := &queue.RedisQueue{Client: rdb, Prefix: "matcher-pagination-test"}
	now := time.Now().UTC()
	require.NoError(t, q.Enqueue(ctx, target.ID, "Duo", "eu", sessA.ID, now))
	require.NoError(t, q.Enqueue(ctx, target.ID, "Duo", "eu", sessB.ID, now.Add(time.Millisecond)))

	events := &recordingMatchEvents{}
	worker := &matcher.Worker{
		Queue:    q,
		Sessions: sessions,
		Matches:  matches,
		Games:    games,
		Bans:     &store.BanStore{Pool: pool},
		Events:   events,
	}
	require.NoError(t, worker.RunOnce(ctx))

	updatedA, err := sessions.Get(ctx, sessA.ID)
	require.NoError(t, err)
	updatedB, err := sessions.Get(ctx, sessB.ID)
	require.NoError(t, err)
	require.Equal(t, store.SessionStatusPendingAccept, updatedA.Status)
	require.Equal(t, store.SessionStatusPendingAccept, updatedB.Status)
	require.NotNil(t, updatedA.MatchID)
	require.Equal(t, updatedA.MatchID, updatedB.MatchID)

	var matchCount int
	err = pool.QueryRow(ctx, `SELECT count(*) FROM matches WHERE game_id = $1`, target.ID).Scan(&matchCount)
	require.NoError(t, err)
	require.Equal(t, 1, matchCount, "one match proposal must be created for the target game")

	proposals, err := matches.ListProposals(ctx, *updatedA.MatchID)
	require.NoError(t, err)
	require.Len(t, proposals, 2)
	for _, proposal := range proposals {
		require.Equal(t, store.ProposalResponsePending, proposal.Response)
	}
	require.Equal(t, 1, events.matchFound)
}
