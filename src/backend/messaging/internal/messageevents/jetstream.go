...
// jetStreamClient deliberately excludes stream administration. Bootstrap owns it.
...
// EnsureStream validates that bootstrap has provisioned the exact stream.
...
func (p *JetStreamPublisher) ensureStream() error {
	...
	p.ensureOnce.Do(func() {
		...
		p.ensureErr = validateBootstrappedStream(info, messageEventStreamSubjects())
	})
	return p.ensureErr
}

func validateBootstrappedStream(info *nats.StreamInfo, expected []string) error {
