...
func LoadFromEnv() (Config, bool, error) {
	...
	cfg := Config{
		...
		TLSCertFile:    strings.TrimSpace(os.Getenv("SUBSCRIPTION_PRINCIPAL_TLS_CERT_FILE")),
		TLSKeyFile:     strings.TrimSpace(os.Getenv("SUBSCRIPTION_PRINCIPAL_TLS_KEY_FILE")),
		ListenAddr:     ":9091",
	}
	if value, ok := os.LookupEnv("SUBSCRIPTION_PRINCIPAL_GRPC_LISTEN"); ok {
