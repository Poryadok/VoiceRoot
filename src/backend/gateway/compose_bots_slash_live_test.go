...
// TestComposeBotsSlash_live registers a polling bot and verifies /ping returns pong in channel history.
func TestComposeBotsSlash_live(t *testing.T) {
...
// TestComposeBotsEphemeral_live verifies ephemeral slash: invoker sees content,
// no persisted message in response or chat history; other members see nothing.
func TestComposeBotsEphemeral_live(t *testing.T) {
	if !liveComposeEnabled() {
		t.Skip("set VOICE_RUN_LIVE_COMPOSE=true to run against local compose")
	}
	clearLiveComposeAuthRateLimit(t)
...
// TestComposeBotsTimeout_live returns error_code bot_timeout when polling bot never completes.
func TestComposeBotsTimeout_live(t *testing.T) {
	if !liveComposeEnabled() {
		t.Skip("set VOICE_RUN_LIVE_COMPOSE=true to run against local compose")
	}
	clearLiveComposeAuthRateLimit(t)
...
// TestComposeBotsWebhook_live exercises webhook delivery via host.docker.internal.
func TestComposeBotsWebhook_live(t *testing.T) {
	if !liveComposeEnabled() {
		t.Skip("set VOICE_RUN_LIVE_COMPOSE=true to run against local compose")
	}
	clearLiveComposeAuthRateLimit(t)
...
// TestComposeBotsSlashDeferred_live verifies defer then async SendBotMessage follow-up.
func TestComposeBotsSlashDeferred_live(t *testing.T) {
	if !liveComposeEnabled() {
		t.Skip("set VOICE_RUN_LIVE_COMPOSE=true to run against local compose")
	}
	clearLiveComposeAuthRateLimit(t)
...
// TestComposeBotsOfflineGreyout_live lists slash commands after install without polling;
// bot must appear offline (online:false) until presence heartbeat.
func TestComposeBotsOfflineGreyout_live(t *testing.T) {
	if !liveComposeEnabled() {
		t.Skip("set VOICE_RUN_LIVE_COMPOSE=true to run against local compose")
	}
	clearLiveComposeAuthRateLimit(t)
...
// TestComposeBotsPerChatToggle_live disables a bot in one chat via REST PATCH
// while slash commands remain available in another whitelisted chat (BOT-B).
func TestComposeBotsPerChatToggle_live(t *testing.T) {
	if !liveComposeEnabled() {
		t.Skip("set VOICE_RUN_LIVE_COMPOSE=true to run against local compose")
	}
	clearLiveComposeAuthRateLimit(t)
...
// TestComposeBotsPrivilegedInstall_live rejects installing a history bot without owner ack.
func TestComposeBotsPrivilegedInstall_live(t *testing.T) {
	if !liveComposeEnabled() {
		t.Skip("set VOICE_RUN_LIVE_COMPOSE=true to run against local compose")
	}
	clearLiveComposeAuthRateLimit(t)
...
// TestComposeBotsBotCRoutes_live exercises BOT-C bot-token REST routes:
// presence heartbeat, space member list, and create chat in space.
func TestComposeBotsBotCRoutes_live(t *testing.T) {
	if !liveComposeEnabled() {
		t.Skip("set VOICE_RUN_LIVE_COMPOSE=true to run against local compose")
	}
	clearLiveComposeAuthRateLimit(t)
...
// TestComposeBotsDailyChatCreateLimit_live enforces 10 text chat creates per day per bot (bots.md).
func TestComposeBotsDailyChatCreateLimit_live(t *testing.T) {
	if !liveComposeEnabled() {
		t.Skip("set VOICE_RUN_LIVE_COMPOSE=true to run against local compose")
	}
	clearLiveComposeAuthRateLimit(t)
