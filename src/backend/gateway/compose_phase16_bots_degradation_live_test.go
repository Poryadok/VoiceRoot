...
// TestComposePhase16BotsBotCWhenBotDown_live verifies BOT-C routes degrade when bot upstream is absent.
func TestComposePhase16BotsBotCWhenBotDown_live(t *testing.T) {
	if !liveComposeEnabled() {
		t.Skip("set VOICE_RUN_LIVE_COMPOSE=true to run against local compose")
	}
