...
func TestValidateBootstrappedStream(t *testing.T) {
	valid := &nats.StreamInfo{Config: nats.StreamConfig{Name: streamName, Subjects: storyEventStreamSubjects(), Retention: nats.LimitsPolicy, MaxAge: 7 * 24 * time.Hour, Storage: nats.FileStorage}}
	require.NoError(t, validateBootstrappedStream(valid))
	require.Error(t, validateBootstrappedStream(nil))
	valid.Config.MaxAge = time.Hour
...
func TestPublisherFailsClosedWhenStreamIsUnavailable(t *testing.T) {
	...
	require.Error(t, publisher.PublishStoryCreated(t.Context(), "story", "author", "text", "", nil))
}
