...
func LoadFromEnv() (Config, bool, error) {
	names := []string{"FILE_PRINCIPAL_GRPC_LISTEN", "FILE_PRINCIPAL_TLS_CERT_FILE", "FILE_PRINCIPAL_TLS_KEY_FILE", "FILE_PRINCIPAL_REPLAY_REDIS_ADDR", "FILE_PRINCIPAL_REPLAY_REDIS_PASSWORD", "S2S_JWKS_URLS_JSON", "S2S_JWKS_REFRESH_AFTER", "S2S_JWKS_HARD_EXPIRY", "S2S_UNKNOWN_KID_COOLDOWN", "S2S_JWKS_CA_FILE"}
	enabled := false
	...
	cfg := Config{
		...
		JWKSCAFile: strings.TrimSpace(os.Getenv("S2S_JWKS_CA_FILE")), TLSCertFile: strings.TrimSpace(os.Getenv("FILE_PRINCIPAL_TLS_CERT_FILE")), TLSKeyFile: strings.TrimSpace(os.Getenv("FILE_PRINCIPAL_TLS_KEY_FILE")),
