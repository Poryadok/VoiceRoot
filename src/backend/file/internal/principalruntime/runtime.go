...
func New(ctx context.Context, cfg Config) (*Runtime, error) {
	...
	fetch := func(ctx context.Context, issuer string) ([]byte, error) {
		endpoint, trusted := cfg.JWKSURLs[issuer]
		if !trusted {
...
func (r *Runtime) Verify(ctx context.Context, token, method, requestID, hash string) (principal.Principal, error) {
	...
	if !principalgrpc.IsProtectedMethod(method) {
		return principal.Principal{}, errors.New("principal method is not allowed")
	}
	parts := strings.Split(token, ".")
	if len(parts) != 3 {
		return principal.Principal{}, errors.New("invalid principal")
	}
	...
	if json.Unmarshal(payload, &hint) != nil || r.issuers[hint.Issuer] == "" {
		return principal.Principal{}, errors.New("untrusted principal issuer")
	}
	// The untrusted issuer only selects a configured key set. Verification binds
	// the signature and the same issuer before it can create any authority.
	p, err := principal.VerifyService(ctx, token, principal.VerifyConfig{ExpectedIssuer: hint.Issuer, ExpectedAudience: "file", ExpectedRPC: method, ExpectedRequestID: requestID, ExpectedRequestHash: hash, KeyResolver: r.resolver.Resolve, ReplayGuard: r.recordReplay})
	if err != nil {
		return principal.Principal{}, err
	}
...
func (r *Runtime) recordReplay(ctx context.Context, issuer, jwtID string, expiresAt time.Time) error {
	...
	result, err := r.replay.SetArgs(ctx, fmt.Sprintf("file:principal:replay:%x", digest), "1", redis.SetArgs{Mode: "NX", TTL: ttl}).Result()
