...
func LoadFromEnv() (Config, bool, error) {
	names := []string{"SEARCH_PRINCIPAL_GRPC_LISTEN", "SEARCH_PRINCIPAL_TLS_CERT_FILE", "SEARCH_PRINCIPAL_TLS_KEY_FILE", "SEARCH_PRINCIPAL_REPLAY_REDIS_ADDR", "SEARCH_PRINCIPAL_REPLAY_REDIS_PASSWORD", "S2S_JWKS_URLS_JSON", "S2S_JWKS_REFRESH_AFTER", "S2S_JWKS_HARD_EXPIRY", "S2S_UNKNOWN_KID_COOLDOWN", "S2S_JWKS_CA_FILE"}
	enabled := false
	...
	cfg := Config{
		ReplayAddr: strings.TrimSpace(os.Getenv("SEARCH_PRINCIPAL_REPLAY_REDIS_ADDR")), ReplayPassword: os.Getenv("SEARCH_PRINCIPAL_REPLAY_REDIS_PASSWORD"),
		JWKSCAFile: strings.TrimSpace(os.Getenv("S2S_JWKS_CA_FILE")), TLSCertFile: strings.TrimSpace(os.Getenv("SEARCH_PRINCIPAL_TLS_CERT_FILE")), TLSKeyFile: strings.TrimSpace(os.Getenv("SEARCH_PRINCIPAL_TLS_KEY_FILE")),
		ListenAddr: strings.TrimSpace(os.Getenv("SEARCH_PRINCIPAL_GRPC_LISTEN")),
	}
...
func (c Config) validate() error {
	...
	if len(c.JWKSURLs) != 1 || c.JWKSURLs["space"] == "" {
		return errors.New("exact trusted Space JWKS endpoint required")
	}
