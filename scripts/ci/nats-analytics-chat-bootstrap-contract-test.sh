...
grep -Fq 'deliver_group: $group' "$BOOTSTRAP" || fail 'bootstrap must preserve Analytics queue groups in exact CREATE payload'
...
grep -Fq 'nats.CustomInboxPrefix("_INBOX.voice.analytics")' "$ANALYTICS" || fail 'Analytics JetStream API replies must use the service-scoped inbox prefix'
