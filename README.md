...
Когда репозиторий подключён к **GitHub Actions**, на PR в `master` дополнительно гоняются те же проверки на раннере плюс, при настроенном репозитории, проверка ссылок в `docs/`; выкат на staging — по [DEPLOYMENT.md](docs/DEPLOYMENT.md), отдельно от `make build-all`. Публичные FQDN staging — [`deploy/staging/domains.defaults`](deploy/staging/domains.defaults) (переменные `VOICE_GATEWAY_INGRESS_HOST` / `VOICE_DEVELOPER_PORTAL_INGRESS_HOST` в GitHub, DNS — в `DEPLOYMENT.md`).