...
- Полнотекстовый поиск — PostgreSQL `tsvector` / `GIN`-индекс (или Elasticsearch при росте нагрузки)
- **Языки**: конфигурация `russian` + `english` с авто-определением языка; на старте допустимо `simple` (без стемминга, работает для любого языка)
