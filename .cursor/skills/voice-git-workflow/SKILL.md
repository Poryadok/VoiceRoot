...
Git-операции в Voice следуют **`docs/CONTRIBUTING.md`** и **`.cursor/rules/git-safety.mdc`** (always apply). Этот skill — практический чеклист для агентов; при конфликте приоритет у CONTRIBUTING.

**Worktree / параллельные агенты** — отдельный skill: `.cursor/skills/treehouse/SKILL.md` (treehouse) или встроенный Cursor `/worktree`. Здесь — git **внутри** checkout/worktree.
...
- После падения pre-commit / pre-push / Cursor hook
...
Voice **не переписывает историю** — ни локально, ни на remote. Хуки и Cursor `beforeShellExecution` это блокируют.
...
| Удалять worktree | После merge PR или явного abandon (treehouse `return` / Cursor cleanup) |
...
| Правило агентов | `.cursor/rules/git-safety.mdc` |
| Тесты перед PR | `docs/TESTING.md` |
| Worktree pool | `.cursor/skills/treehouse/SKILL.md` |
| Split больших изменений | `~/.cursor/skills-cursor/split-to-prs/SKILL.md` |
